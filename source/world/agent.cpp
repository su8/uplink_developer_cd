...
bool Agent::ParseAccessCode ( const char *thecode, char *username, char *password )
{
...
int Agent::HasAccount  ( char *ip )
{
    ...
    char *code = codes.GetData (ip);
    if ( !code ) return -1;

    char username [64];
    char password [64];
    ParseAccessCode ( code, username, password );
...
void Agent::GiveCode ( char *newip, char *newcode )
{
        ...
        if ( ips->ValidIndex (i) && thecodes->ValidIndex (i) ) {
            ...
            if ( strcmp ( thisip, newip ) == 0 ) {
                ...
                if ( strcmp ( thiscode, newcode ) == 0 ) {
                    ...
                    char thispassword [64];

                    bool successA = Player::ParseAccessCode ( newcode, newusername, newpassword );
                    bool successB = Player::ParseAccessCode ( thiscode, thisusername, thispassword );
    ...
    if ( !found ) {
	    ...
	    char *newcodecopy = new char [128];
	    strcpy ( newcodecopy, newcode );
...
void Agent::AttemptMission_TraceUser ()
{
    ...
    if ( strcmp ( hackername, "PLAYER" ) == 0 ) {

        char realbuffer [9];
        strcpy ( realbuffer, "\xaa\xab\x15\xdd\xdd\xee\xe9\x2d" );
        ...
        readbuffer [8] = '\x0';
        int match = ( strcmp ( readbuffer, realbuffer ) == 0 );
        fclose ( file );
				...
				if ( traced_ip ) {
					...
					if ( strcmp ( hacker->localhost, traced_ip ) == 0 ) {
						...
						if ( framed ) {
							...
							strcpy ( m->completionA, framed->name );
...
void Agent::Load ( FILE *file )
{
	...
	LoadDynamicString ( handle, SIZE_AGENT_HANDLE, file );

	LoadLList ( &links, file );
	LoadBTree ( &codes, file );
	LoadLList ( (LList <UplinkObject *> *) &missions, file );
