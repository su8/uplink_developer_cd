...
void LogBank::Load ( FILE *file )
{
	...
	for ( int i = 0; i < size; ++i ) {
		...
		bool modified;
		fread ( &modified, sizeof(modified), 1, file );
		...
		if ( modified ) {
            ...
            if ( index != -1 ) {
			    ...
			    AccessLog *al = new AccessLog ();
                if ( al ) al->SetProperties ( logs.GetData (index) );
			    internallogs.PutData ( al, index );
...
void AccessLog::Load ( FILE *file )
{
	...
	LoadDynamicString ( fromip, SIZE_VLOCATION_IP, file );
	LoadDynamicString ( fromname, SIZE_PERSON_NAME, file );

	fread ( &TYPE, sizeof(TYPE), 1, file );
	fread ( &SUSPICIOUS, sizeof(SUSPICIOUS), 1, file );

	data1 = LoadDynamicString ( file );
	data2 = LoadDynamicString ( file );
	data3 = LoadDynamicString ( file );
...
void AccessLog::Save ( FILE *file )
{
	...
	SaveDynamicString ( fromip, file );
	SaveDynamicString ( fromname, file );
