...
bool LanMonitor::IsAccessable ( int newNode )
{
    ...
    if ( newSystem->TYPE == LANSYSTEM_MAINSERVER ) {
        ...
        if ( newSystem->data1 != -1 && lanComputer->systems.GetData(newSystem->data1)->data1 == 1 ) return false;
        if ( newSystem->data2 != -1 && lanComputer->systems.GetData(newSystem->data2)->data1 == 1 ) return false;
        if ( newSystem->data3 != -1 && lanComputer->systems.GetData(newSystem->data3)->data1 == 1 ) return false;
...
void LanMonitor::SysAdminAwaken ()
{
    ...
    if ( sysAdminState < SYSADMIN_CURIOUS ) {
        ...
        int timeToDiscover = NumberGenerator::RandomNormalNumber( 10, 5 );
        sysAdminTimer = EclGetAccurateTime() + timeToDiscover * 1000;
...
void LanMonitor::Update ()
{
    ...
    switch ( sysAdminState )
    {
        ...
        case SYSADMIN_FOUNDYOU:
        {
		    ...
		    duedate.AdvanceSecond ( NumberGenerator::RandomNormalNumber ( 30, 10 ) );
