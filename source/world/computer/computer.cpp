...
void Computer::Load  ( FILE *file )
{
	...
	LoadDynamicString ( name, SIZE_COMPUTER_NAME, file );
	LoadDynamicString ( companyname, SIZE_COMPANY_NAME, file );
	LoadDynamicString ( ip, SIZE_VLOCATION_IP, file );

	fread ( &tracespeed, sizeof(tracespeed), 1, file );
	fread ( &traceaction, sizeof(traceaction), 1, file );

	fread ( &numrecenthacks, sizeof(numrecenthacks), 1, file );
	fread ( &numhacksthismonth, sizeof(numhacksthismonth), 1, file );
	fread ( &numhackslastmonth, sizeof(numhackslastmonth), 1, file );

	fread ( &istargetable, sizeof(istargetable), 1, file );
	fread ( &isexternallyopen, sizeof(isexternallyopen), 1, file );
	fread ( &isrunning, sizeof(isrunning), 1, file );
	fread ( &isinfected_revelation, sizeof(isinfected_revelation), 1, file );

	LoadDArray ( (DArray <UplinkObject *> *) &screens, file );	

	databank.Load ( file );
	logbank.Load ( file );
	recordbank.Load ( file );
	security.Load ( file );
    infectiondate.Load ( file );
...
void Computer::Save  ( FILE *file )
{
	...
	SaveDynamicString ( companyname, file );
...
void Computer::GenerateAccessCode( char *code, char *result )
{
    sprintf( result, "%s:'%s'", "CODE", code );
}

void Computer::GenerateAccessCode( char *name, char *code, char *result )
{
    sprintf( result, "%s:'%s', %s:'%s'", "NAME", name, "CODE", code );
}

void Computer::GenerateAccessCode( int accNo, char *code, char *result )
{
    sprintf( result, "%s:'%d', %s:'%s'", "NAME", accNo, "CODE", code );
}
