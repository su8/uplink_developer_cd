...
int BankComputer::GenerateUniqueAccountNumber ()
{
	...
	sprintf ( previousaccno_s, "%d", previousaccno );
...
int BankComputer::CreateBankAccount ( BankAccount *newaccount )
{
	...
	char s_account [16];
	sprintf ( s_account, "%d", newaccount->accountnumber );
...
void BankComputer::Load ( FILE *file )
{
	...
	fread ( &previousaccno, sizeof(previousaccno), 1, file );

	LoadBTree ( (BTree <UplinkObject *> *) &accounts, file );	
