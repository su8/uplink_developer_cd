...
char *Date::GetShortString ()
{
	...
	sprintf ( result, "%.2d:%.2d, %d-%d-%d", hour, minute, day, month, year ); 
	strcpy ( tempdate, result );
    return tempdate;
...
char *Date::GetLongString ()
{
	...
	sprintf ( result, "%.2d:%.2d.%.2d, %d %s %d", hour, minute, second, day, GetMonthName (month), year ); 
    strcpy ( tempdate, result );
	return tempdate;
...
void Date::Load ( FILE *file )
{

	LoadID ( file );

	fread ( &second, sizeof (second), 1, file );
	fread ( &minute, sizeof (minute), 1, file );
	fread ( &hour,   sizeof (hour),   1, file );
	fread ( &day,    sizeof (day),    1, file );
	fread ( &month,  sizeof (month),  1, file );
	fread ( &year,   sizeof (year),   1, file );

	fread ( &updateme, sizeof (updateme), 1, file );
...
void Date::Update ()
{
	...
	if ( updateme ) {									// Update the time
		...
		int nummilliseconds = EclGetAccurateTime () - previousupdate;
		...
		if ( game->GameSpeed () == GAMESPEED_NORMAL ) {
			...
			if ( nummilliseconds > 1000 ) {
				...
				AdvanceSecond ( 1 );				
				previousupdate = EclGetAccurateTime ();
