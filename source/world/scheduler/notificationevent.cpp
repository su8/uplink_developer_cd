...
void NotificationEvent::Run ()
{
	...
	switch ( TYPE ) {
		...
		default:
			UplinkAbort ( "Unrecognised notification type" );
        ...
        if (account) {
		    ...
		    if ( account->loan > 0 ) {
			    ...
			    if ( account->loan <= SMALLLOAN_MAX ) 
				    amountpaid = ( account->loan * SMALLLOAN_INTEREST ) / 12;

			    else if ( account->loan <= MEDIUMLOAN_MAX ) 
				    amountpaid += ( account->loan * MEDIUMLOAN_INTEREST ) / 12;

			    else if ( account->loan <= LARGELOAN_MAX )
				    amountpaid += ( account->loan * LARGELOAN_INTEREST ) / 12;

			    else
				    amountpaid += ( account->loan * MAXLOAN_INTEREST ) / 12;
...
void NotificationEvent::GiveMissionToNPC ()
{
	...
	if ( prioritymission ) {
		...
		if ( nummissions > 0 ) {
			...
			int missionindex = NumberGenerator::RandomNormalNumber ( nummissions / 4, nummissions / 2 );
...
void NotificationEvent::Load  ( FILE *file )
{
