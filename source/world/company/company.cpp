...
Company::Company()
{
	...
	sprintf ( name, " " );
	sprintf ( boss, "Unlisted" );
	sprintf ( administrator, "Unlisted" );
	size=0;
	TYPE=0;
	growth=0;
	alignment=0;
...
void Company::SetAdmin ( char *adminname )
{
	...
	strcpy ( administrator, adminname );
...
void Company::Grow ( int dayspast )
{
	...
	size *= (100.0 + (fraction_of_growth * growth)) / 100.0;
...
int Company::GetShareChange ()
{
	...
	int percentchange = 100 * (float) ( currentprice - oldprice ) / (float) oldprice;
...
void Company::Load  ( FILE *file )
{

	LoadID ( file );

	LoadDynamicString ( name, SIZE_COMPANY_NAME, file );
	LoadDynamicString ( boss, SIZE_PERSON_NAME, file );
	LoadDynamicString ( administrator, SIZE_PERSON_NAME, file );

	fread ( &size, sizeof(size), 1, file );
	fread ( &TYPE, sizeof(TYPE), 1, file );
	fread ( &growth, sizeof(growth), 1, file );
	fread ( &alignment, sizeof(alignment), 1, file );

	for ( int i = 0; i < 12; ++i ) 
		fread ( &sharehistory [i], sizeof(sharehistory [i]), 1, file );

	fread ( &lastmonthset, sizeof(lastmonthset), 1, file );
...
void Company::Save  ( FILE *file )
{
	...
	SaveDynamicString ( boss, file );
	SaveDynamicString ( administrator, file );
