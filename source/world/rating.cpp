...
void Rating::ChangeNeuromancerScore ( int amount )
{
	...
	if ( neuromancerrating > oldrating ) {
		...
		if ( game->GetWorld ()->GetPerson (owner) ) {
			...
			char body [128];
			sprintf ( body, "Agent %s - your Neuromancer rating appears to have changed.\n\n"
		...
		if ( game->GetWorld ()->GetPerson (owner) ) {
			...
			char body [256];
			sprintf ( body, "It appears your neuromancer rating has changed, in view of your recent actions.\n\n"
...
void Rating::Load  ( FILE *file )
{

	LoadID ( file );

	LoadDynamicString ( owner, SIZE_PERSON_NAME, file );

	fread ( &uplinkscore, sizeof(uplinkscore), 1, file );
	fread ( &neuromancerscore, sizeof(neuromancerscore), 1, file );
	fread ( &uplinkrating, sizeof(uplinkrating), 1, file );
	fread ( &neuromancerrating, sizeof(neuromancerrating), 1, file );
	fread ( &creditrating, sizeof(creditrating), 1, file );
