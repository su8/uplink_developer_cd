...
void Connection::AddVLocation ( char *ip )
{
	...
	char *newip = new char [SIZE_VLOCATION_IP];
...
void Connection::Load ( FILE *file )
{

	LoadID ( file );

	fread ( owner, sizeof(owner), 1, file );
	fread ( &traceinprogress, sizeof(traceinprogress), 1, file );
	fread ( &traceprogress, sizeof(traceprogress), 1, file );

	LoadLList ( &vlocations, file );
