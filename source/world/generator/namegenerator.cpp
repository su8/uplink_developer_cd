...
static char tempname [256];                                 // This is used to return string values
...
//	char filename [128];
//	sprintf ( filename, "%sdata.zip", app->path );

//	RsLoadArchive ( "data.zip", app->path );
...
char *NameGenerator::GenerateCompanyName ()
{
	...
	sprintf ( name, "%s %s", companynameA, companynameB );
...
char *NameGenerator::GeneratePersonName ()
{
	...
	sprintf ( name, "%s %s", forname, surname );
...
char *NameGenerator::GenerateDataName ( char *companyname, int DATATYPE )
{
	...
	switch ( DATATYPE ) {
		...
		case DATATYPE_NONE:
		{
			...
			sprintf ( dataname, "%c%c%c-file-%d", companyname [0], companyname [1], companyname [2],
		...
		case DATATYPE_DATA:
		{
			...
			sprintf ( dataname, "%c%c%c-data-%d", companyname [0], companyname [1], companyname [2],
													NumberGenerator::RandomNumber ( 99999 ) );
			break;
		...
		case DATATYPE_PROGRAM:
		{
			...
			sprintf ( dataname, "%c%c%c-prog-%d", companyname [0], companyname [1], companyname [2],
													NumberGenerator::RandomNumber ( 99999 ) );
			break;
