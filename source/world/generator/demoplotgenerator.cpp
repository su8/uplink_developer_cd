...
Mission *DemoPlotGenerator::Generate_ChangeSocialRecordARC ()
{
	...
	sprintf ( personname, "internal@ARC.net" );	

	char description [SIZE_MISSION_DESCRIPTION];
	ostrstream details;
	ostrstream fulldetails;

    sprintf ( description, "Modify a protected Social Security document" );
	...
	strcpy ( completionC, "Social Security" );
	sprintf ( completionD, "%d", newnumber );						
	sprintf ( completionE, "%d", newnumber );

	strcpy ( completionA, target->ip );
	strcpy ( completionB, person->name );
    
    strcpy ( arcmissiontarget, person->name );
...
void DemoPlotGenerator::Load  ( FILE *file )
{
