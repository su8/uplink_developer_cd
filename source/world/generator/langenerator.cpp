...
//	game->GetWorld()->CreateCompany( companyName );
	...
	while ( thefile )
	{
        ...
        else if ( strstr(systemName, "MODEM" ) )            indexUsed = GenerateModem               (comp, cluster, x, y, security, dataX);
        else if ( strstr(systemName, "LOGSERVER" ) )        indexUsed = GenerateLogServer           (comp, cluster, x, y, security);

		else 
			printf ( "LanGenerator::LoadLAN WARNING: Unrecognised System TYPE %s\n(%s)\n", systemName, filename );
	...
	comp->databank.SetSize ( NumberGenerator::RandomNormalNumber ( 100, 40 ) );
	...
	for ( i = 0; i < numfiles; ++i ) {
		...
		int size = NumberGenerator::RandomNormalNumber ( 6, 4 );		
...
void LanGenerator::GenerateLANCluster ( LanComputer *comp, int difficulty )
{
    ...
    switch ( difficulty ) {
		...
		case 0 :		GenerateLAN_Level0Cluster ( comp, cluster, 10, 100, 200, 200 );       break;
		case 1 :		GenerateLAN_Level1Cluster ( comp, cluster, 10, 100, 275, 275 );       break;
        case 2 :        GenerateLAN_Level2Cluster ( comp, cluster );                          break;      
        case 3 :        GenerateLAN_Level3Cluster ( comp, cluster );                          break;
        case 4 :        GenerateLAN_Level4Cluster ( comp, cluster );                          break;
...
void LanGenerator::GenerateLAN_Level0Cluster ( LanComputer *comp, LanCluster *cluster, int x, int y, int width, int height )
{
    ...
    if ( type == 1 ) {
        ...
        cluster->AddLanLink ( hubIndex, 0.2, 1.0, t1Index, 0.5, 0.0, 1 );
        cluster->AddLanLink ( hubIndex, 0.4, 1.0, t2Index, 0.5, 0.0, 1 );
        cluster->AddLanLink ( hubIndex, 0.6, 1.0, t3Index, 0.5, 0.0, 1 );
        cluster->AddLanLink ( hubIndex, 0.8, 1.0, t4Index, 0.5, 0.0, 1 );
...
void LanGenerator::GenerateLAN_Level1Cluster ( LanComputer *comp, LanCluster *cluster, int x, int y, int width, int height )
{
    ...
    if ( type == 1 ) {
        ...
        cluster->AddLanLink ( hubIndex, 0.2, 1.0, lock1Index, 0.5, 0.0, 1 );
        cluster->AddLanLink ( hubIndex, 0.5, 1.0, authIndex, 0.5, 0.0, 1 );
        cluster->AddLanLink ( lock1Index, 0.5, 1.0, isoIndex, 0.0, 0.5, 1 );
        cluster->AddLanLink ( hubIndex, 1.0, 0.5, lock2Index, 0.5, 0.0, 1 );
        cluster->AddLanLink ( lock2Index, 0.0, 0.5, isoIndex, 1.0, 0.5, 1 );
        ...
        recei->data1 = RADIOTRANSMITTER_MINRANGE + NumberGenerator::RandomNumber( RADIOTRANSMITTER_MAXRANGE - RADIOTRANSMITTER_MINRANGE );
        recei->data2 = (float) NumberGenerator::RandomNumber ( 10 );

        LanComputerSystem *term = cluster->systems.GetData( termIndex );
        char freqData [128];
        sprintf ( freqData, "Radio Receiver Frequency : %d.%d Ghz", recei->data1, recei->data2 );
        HideData ( comp, term, freqData );

        cluster->AddLanLink ( hubIndex, 0.0, 0.5, termIndex, 1.0, 0.5, 1 );
        cluster->AddLanLink ( hubIndex, 0.3, 1.0, authIndex, 0.5, 0.0, 1 );
        cluster->AddLanLink ( hubIndex, 0.7, 1.0, lockIndex, 0.5, 0.0, 1 );
        cluster->AddLanLink ( lockIndex, 0.5, 1.0, transIndex, 0.5, 0.0, 1 );
        ...
        {
            case 0 : term = cluster->systems.GetData( t1Index );
            case 1 : term = cluster->systems.GetData( t2Index );
            case 2 : term = cluster->systems.GetData( t3Index );
            default: term = cluster->systems.GetData( t3Index );
        }
        HideData ( comp, term, freqData );
        ...
        char phoneNum[32];
        sprintf( phoneNum, "UnInitialised" );
        ...
        int modemIndex = GenerateModem ( comp, cluster, x3, y, 3, phoneNum );
        ...
        char data [128];
        sprintf ( data, "Local Dial-up number : %s", phoneNum );
...
void LanGenerator::GenerateLAN_Level2Cluster ( LanComputer *comp, LanCluster *cluster )
{
    ...
    if ( type == 1 ) {
        ...
        GenerateLAN_Level0Cluster ( comp, cluster, 0, 50, 200, 200 );
        ...
        GenerateLAN_Level1Cluster ( comp, cluster, 250, 50, 275, 275 );
        ...
        GenerateLAN_Level1Cluster ( comp, cluster, 0, 0, 275, 275 );
        int input1 = cluster->input;
        int output1 = cluster->output;

        LanCluster *cluster1 = new LanCluster ();
        GenerateLAN_Level0Cluster ( comp, cluster1, 325, 0, 200, 200 );
        cluster1->Rotate ( 3 );
        ...
        cluster->AddLanLink ( hubIndex, 0.1, 1.0, input1, 0.5, 0.0, 1 );
        cluster->AddLanLink ( hubIndex, 0.3, 1.0, input2, 0.5, 0.0, 1 );
        cluster->AddLanLink ( hubIndex, 0.5, 1.0, input3, 0.5, 0.0, 1 );
        cluster->AddLanLink ( hubIndex, 1.0, 0.5, msIndex, 0.0, 0.5, 1 );
...
void LanGenerator::GenerateLAN_Level3Cluster ( LanComputer *comp, LanCluster *cluster )
{
    // TODO : Fill this in
    GenerateLAN_Level4Cluster ( comp, cluster );

}

void LanGenerator::GenerateLAN_Level4Cluster ( LanComputer *comp, LanCluster *cluster )
{
    ...
    if ( type == 1 ) {
        ...
        GenerateLAN_Level0Cluster ( comp, part2, 325, 0, 200, 200 );
        part2->Rotate( 3 );
        ...
        GenerateLAN_Level0Cluster ( comp, cluster, 550, 200, 200, 200 );
        int input3 = cluster->input;
        int output3 = cluster->output;
        ...
        GenerateLAN_Level1Cluster ( comp, part4, 800, 350, 275, 275 );
        part4->Rotate( 3 );
        ...
        GenerateLAN_Level1Cluster ( comp, cluster, 0, 50, 275, 275 );
        int input1 = cluster->input;
        int output1 = cluster->output;
        ...
        GenerateLAN_Level1Cluster ( comp, cluster, 330, 300, 300, 300 );
        int input2 = cluster->input;
        int output2 = cluster->output;
        char phoneNum[64];
        sprintf( phoneNum, "UnInitialised" );
        int modemIndex = GenerateModem ( comp, cluster, 480, 100, 3, phoneNum );
        ...
        GenerateLAN_Level1Cluster ( comp, cluster, 660, 50, 275, 275 );
        int input3 = cluster->input;
        int output3 = cluster->output;
    ...
    if ( type == 3 ) {
        ...
        GenerateLAN_Level1Cluster ( comp, leftCluster, 0, 100, 300, 300 );
        ...
        recei1->data1 = RADIOTRANSMITTER_MINRANGE + NumberGenerator::RandomNumber( RADIOTRANSMITTER_MAXRANGE - RADIOTRANSMITTER_MINRANGE );
        recei1->data2 = (float) NumberGenerator::RandomNumber ( 10 );
        ...
        GenerateLAN_Level1Cluster ( comp, rightCluster, 900, 60, 300, 300 );
        ...
        recei2->data1 = RADIOTRANSMITTER_MINRANGE + NumberGenerator::RandomNumber( RADIOTRANSMITTER_MAXRANGE - RADIOTRANSMITTER_MINRANGE );
        recei2->data2 = (float) NumberGenerator::RandomNumber ( 10 );
        ...
        GenerateLAN_Level1Cluster ( comp, lowerCluster, 460, 500, 300, 300 );
        ...
        recei3->data1 = RADIOTRANSMITTER_MINRANGE + NumberGenerator::RandomNumber( RADIOTRANSMITTER_MAXRANGE - RADIOTRANSMITTER_MINRANGE );
        recei3->data2 = (float) NumberGenerator::RandomNumber ( 10 );
        ...
        char data [512];
        sprintf ( data, "Known Radio Receiver Frequencies :\n\n"
                        "Receiver A : %d.%d Ghz\n"
                        "Receiver B : %d.%d Ghz\n"
                        "Receiver C : %d.%d Ghz",
        ...
        cluster->AddLanLink ( hubIndex, 0.0, 0.3, t1Index, 0.5, 0.0, 1 );
        cluster->AddLanLink ( t1Index, 0.5, 1.0, transIndex, 0.0, 0.5, 1 );
        cluster->AddLanLink ( hubIndex, 1.0, 0.3, t2Index, 0.5, 0.0, 1 );
        cluster->AddLanLink ( t2Index, 0.5, 1.0, transIndex, 1.0, 0.5, 1 );

        cluster->AddLanLink ( hubIndex, 0.0, 0.7, l1Index, 0.5, 0.0, 1 );
        cluster->AddLanLink ( hubIndex, 1.0, 0.7, l2Index, 0.5, 0.0, 1 );
        cluster->AddLanLink ( hubIndex, 0.5, 1.0, msIndex, 0.5, 0.0, 1 );
        cluster->AddLanLink ( l1Index, 1.0, 0.5, msIndex, 0.0, 0.5, 1 );
        cluster->AddLanLink ( l2Index, 0.0, 0.5, msIndex, 1.0, 0.5, 1 );
        cluster->AddLanLink ( transIndex, 0.5, 0.0, l3Index, 0.5, 1.0, 1 );
        cluster->AddLanLink ( l3Index, 0.5, 0.0, msIndex, 0.5, 1.0, 1 );
        ...
        GenerateLAN_Level0Cluster ( comp, cluster, 0, 0, 200, 200 );
        int input1 = cluster->input;
        int output1 = cluster->output;
    
        GenerateLAN_Level1Cluster ( comp, cluster, 0, 275, 300, 300 );
        int input2 = cluster->input;
        int output2 = cluster->output;

        LanCluster *part3 = new LanCluster ();
        GenerateLAN_Level1Cluster ( comp, part3, 340, 275, 300, 300 );
        part3->Rotate( 3 );
...
int LanGenerator::GenerateModem ( LanComputer *comp, LanCluster *cluster, int x, int y, int security, char *phoneNum )
{
	...
	char computername [SIZE_COMPUTER_NAME];
	sprintf ( computername, "%s LAN Dial Up access", comp->companyname );
    ...
    if ( phoneNum && phoneNum[0] == '0' )
        ...
        sprintf ( ip, "0%d%d%d%d-%d%d%d%d%d%d", NumberGenerator::RandomNumber (10), NumberGenerator::RandomNumber (10), NumberGenerator::RandomNumber (10), NumberGenerator::RandomNumber (10),
    ...
    if ( phoneNum ) strcpy ( phoneNum, ip );
    ...
    char textMsg[512];
    sprintf ( textMsg,   "LAN DIAL-UP ACCESSED\n\n"
...
bool LanGenerator::HideData ( LanComputer *comp, LanComputerSystem *system, char *data )
{
    ...
    if ( system->dataScreenIndex != -1 ) {
        ...
        if ( cs && cs->GetOBJECTID() == OID_MESSAGESCREEN ) {
            ...
            char *newCaption = new char [ strlen(ms->textmessage) + strlen(data) + 5 ];
            sprintf ( newCaption, "%s\n\n%s", ms->textmessage, data );
