...
void WorldGenerator::Initialise()
{
    ...
    char *filename = app->GetOptions ()->ThemeFilename ( "worldmaplarge_mask.tif" );
...
void WorldGenerator::LoadDynamics ()
{
    ...
    for ( int j = 0; j < extraGateways->Size(); ++j ) {
        if ( extraGateways->ValidIndex(j) ) {
            ...
            def->SetIndex ( numgateways + j + 1 );

            char filename [256];
            char thumbnail [256];
            sprintf ( filename, "gateway/%s", strrchr(gatewayFilename, '/')+1 );
            *(strchr( filename, '.' )) = '\x0';
            strcpy ( thumbnail, filename );
            strcat ( filename, ".tif" );
            strcat ( thumbnail, "_t.tif" );
...
void WorldGenerator::GenerateUplinkPublicAccessServer ()
{
    ...
    game->GetWorld ()->CreateComputer ( compName, "Uplink", IP_UPLINKPUBLICACCESSSERVER );
	
	Computer *comp = game->GetWorld ()->GetComputer ( compName );
	UplinkAssert (comp);
...
void WorldGenerator::GenerateUplinkInternalServicesSystem ()
{
	...
	for ( int i = 0; i < 10; ++i ) {
		...
		char datatitle [64];
		sprintf ( datatitle, "Uplink_Agent_Data %d.dat", i );
        int size = 5;
        int encrypted = 7;
...
void WorldGenerator::GenerateSJGames ()
{
	...
	char contactcomputer [SIZE_COMPUTER_NAME];
	sprintf ( contactcomputer, "Steve Jackson Games Server" );
	Computer *contact = game->GetWorld ()->GetComputer ( contactcomputer );
	UplinkAssert (contact);

	char personname [SIZE_PERSON_NAME];
	sprintf ( personname, "internal@Steve Jackson Games.net" );
	Person *pinternal = game->GetWorld ()->CreatePerson ( personname, contact->ip );
	pinternal->SetIsTargetable ( false );
...
void WorldGenerator::GenerateIntroversion ()
{
	...
	char contactcomputer [SIZE_COMPUTER_NAME];
	sprintf ( contactcomputer, "Introversion Software" );
	Computer *contact = game->GetWorld ()->GetComputer ( contactcomputer );
	UplinkAssert (contact);

	char personname [SIZE_PERSON_NAME];
	sprintf ( personname, "internal@Introversion Software.net" );
	Person *pinternal = game->GetWorld ()->CreatePerson ( personname, contact->ip );
	pinternal->SetIsTargetable ( false );
...
VLocation *WorldGenerator::GenerateLocation ()
{
	...
	sprintf ( ip, "%d.%d.%d.%d", NumberGenerator::RandomNumber (1000), NumberGenerator::RandomNumber (1000), NumberGenerator::RandomNumber (1000), NumberGenerator::RandomNumber (1000) );
...
Company *WorldGenerator::GenerateCompany ()
{
	...
	strcpy ( companyname, NameGenerator::GenerateCompanyName () );
...
Company *WorldGenerator::GenerateCompany_Bank ()
{
	...
	char companyname [SIZE_COMPANY_NAME];
    strcpy ( companyname, NameGenerator::GenerateBankName () );

	int size      = NumberGenerator::RandomNormalNumber ( 4, 4 );
	int growth    = NumberGenerator::RandomNormalNumber ( 10, 20 );
	int alignment = NumberGenerator::RandomNormalNumber ( 0, 50 );
...
Computer *WorldGenerator::GeneratePublicAccessServer ( char *companyname )
{
	...
	comp->SetName ( computername );
	comp->SetCompanyName ( companyname );
	comp->SetTraceSpeed ( NumberGenerator::RandomNormalNumber ( TRACESPEED_PUBLICACCESSSERVER, 
																TRACESPEED_PUBLICACCESSSERVER * TRACESPEED_VARIANCE ) );
	comp->SetIP ( vl->ip );
...
Computer *WorldGenerator::GenerateInternalServicesMachine ( char *companyname )
{
	...
	comp->SetName ( computername );
	comp->SetCompanyName ( companyname );
	comp->SetTraceSpeed ( NumberGenerator::RandomNormalNumber ( TRACESPEED_INTERNALSERVICESMACHINE, 
																TRACESPEED_INTERNALSERVICESMACHINE * TRACESPEED_VARIANCE ) );
	comp->SetIP ( vl->ip );
	...
	uid->SetSubTitle ( "Log in" );
	uid->SetDifficulty ( NumberGenerator::RandomNormalNumber ( HACKDIFFICULTY_INTERNALSERVICESMACHINE, 
															   HACKDIFFICULTY_INTERNALSERVICESMACHINE * HACKDIFFICULTY_VARIANCE ) );
	uid->SetNextPage ( 1 );
	comp->AddComputerScreen ( uid, 0 );
	...
	for ( i = 0; i < numfiles; ++i ) {
		...
		int size = NumberGenerator::RandomNormalNumber ( 6, 6 );		
...
Computer *WorldGenerator::GenerateCentralMainframe ( char *companyname )
{
	...
	comp->SetTraceSpeed ( NumberGenerator::RandomNormalNumber ( TRACESPEED_CENTRALMAINFRAME, 
																TRACESPEED_CENTRALMAINFRAME * TRACESPEED_VARIANCE ) );
	comp->SetTraceAction ( COMPUTER_TRACEACTION_DISCONNECT |
						   COMPUTER_TRACEACTION_LEGAL );
	...
	uid->SetMainTitle ( companyname );
	uid->SetSubTitle ( "Log in" );
	uid->SetDifficulty ( NumberGenerator::RandomNormalNumber ( HACKDIFFICULTY_CENTRALMAINFRAME, 
															   HACKDIFFICULTY_CENTRALMAINFRAME * HACKDIFFICULTY_VARIANCE ) );
	uid->SetNextPage ( 0 );
	comp->AddComputerScreen ( uid, 1 );
...
Person *WorldGenerator::GeneratePerson ()
{
	...
	int age = NumberGenerator::RandomNormalNumber ( 30, 20 );						// 10 - 50
...
Agent *WorldGenerator::GenerateAgent ()
{
    ...
    strcpy ( name, NameGenerator::GeneratePersonName () );
    strcpy ( handle, NameGenerator::GenerateAgentAlias () );
	...
	int age = NumberGenerator::RandomNormalNumber ( 30, 10 );						// 20 - 40
	int photo = NumberGenerator::RandomNumber ( NUM_STARTING_PHOTOS + 1 );
	...
	person->SetRemoteHost ( comp->ip );
    person->SetPhoneNumber ( phone->ip );
	person->rating.SetUplinkRating ( NumberGenerator::RandomNormalNumber ( AGENT_UPLINKRATINGAVERAGE, AGENT_UPLINKRATINGVARIANCE ) );
