...
PlotGenerator::PlotGenerator ()
{
	...
	strcpy ( act1scene3agent, " " );
	strcpy ( act1scene4agent, " " );
	strcpy ( act2scene1agent, " " );
    ...
    strcpy ( saveitforthejury_guytobeframed, "None" );
    strcpy ( saveitforthejury_targetbankip, "None" );
    strcpy ( tracer_lastknownip, "None ");
...
void PlotGenerator::Initialise_ARC ()
{
	...
	sprintf ( personname, "internal@%s.net", companyname );
...
bool PlotGenerator::PlayerContactsARC ( Message *msg )
{
	...
	if ( act == 2 && scene == 1 ) {
		...
		revelation->SetDetails ( DATATYPE_PROGRAM, 4, 0, 0, 1.0, SOFTWARETYPE_OTHER );
...
void PlotGenerator::RunFaith ( char *ip, float version, bool playerresponsible )
{
    ...
    if ( comp->isinfected_revelation &&
        ...
        int percentchance = 100 * (1.0 - (float) (comp->isinfected_revelation - version) / 2.0);
...
void PlotGenerator::Run_Act1Scene2 ()
{
	...
	sprintf ( act1scene3agent, sorted.GetData (1)->name );
	sprintf ( act1scene4agent, sorted.GetData (0)->name );
...
void PlotGenerator::Run_Act1Scene3 ()
{
	...
	UplinkAssert ( strcmp ( act1scene3agent, " " ) != 0 );
...
void PlotGenerator::Run_Act1Scene5 ()
{
	...
	UplinkAssert (hiscomp);

	Data *data = new Data ();
	data->SetTitle ( "RevelationCoreV0.7" );
	data->SetDetails ( DATATYPE_DATA, 4, 5, 0, 0.7 );
...
void PlotGenerator::Run_Act1Scene7 ()
{
	...
	if ( !playercancelsmail ) {
		...
		char from [32];
		sprintf ( from, "Agent %s", agent->handle );
...
void PlotGenerator::Run_Act2Scene1 ()
{
    ...
    char *hiscomputername = strdup(NameGenerator::GeneratePersonalComputerName( act1scene4agent ));
	Computer *hiscomp = game->GetWorld ()->GetComputer ( hiscomputername );
    UplinkAssert (hiscomp);
	char *ip = hiscomp->ip;
...
void PlotGenerator::Run_Act4Scene8 ()
{
    ...
    if ( playerloyalty == -1 ) {
        ...
        version_faith -= 1.0;
        if ( version_faith < 0.2 ) version_faith = 0.2;
...
void PlotGenerator::Run_Act4Scene12 ()
{
	...
	news->SetDetails ( "Arunmor Corporation today released a press statememt that said their primary computer system "
                       ...
                       "system itself.  Arunmor are said to be devastated by this critial error of judgement.\n\n"
...
void PlotGenerator::Run_Act5Scene2 ()
{
    ...
    if ( playerloyalty == -1 ) {
        ...
        int timebetweenlaunches = 3 - (version_faith - version_revelation);
        ...
        int timebetweenlaunches = 3 - (version_revelation - version_faith);
...
Mission *PlotGenerator::GenerateMission_Tracer ()
{
	...
	m->SetPayment ( PAYMENT_SPECIALMISSION_TRACER, PAYMENT_SPECIALMISSION_TRACER * 1.1 );
	m->SetDifficulty ( 10 );
    m->SetMinRating ( 10 );
...
Mission *PlotGenerator::GenerateMission_TakeMeToYourLeader ()
{
	...
	m->SetEmployer ( "Arunmor" );
	m->SetContact ( "internal@Arunmor.net" );
	m->SetPayment ( PAYMENT_SPECIALMISSION_TAKEMETOYOURLEADER, PAYMENT_SPECIALMISSION_TAKEMETOYOURLEADER * 1.1 );
	m->SetDifficulty ( 10 );
    m->SetMinRating ( 10 );
    m->SetAcceptRating ( 99 );    
...
Mission *PlotGenerator::GenerateMission_ARCInfiltration ()
{
	...
	for ( int i = 0; i < numfiles; ++i ) {
		...
		char datatitle [64];
		sprintf ( datatitle, "ARC-REVELATION %d.dat", i );
	...
	sprintf ( description, SpecialMissionDescription (SPECIALMISSION_ARCINFILTRATION) );

	fulldetails << "As you may be aware, ARC are storing most of their Research on their Local Area Network system. "
                   "However, what is not commonly known is that the critial data (ie the research files themselves) "
    ...
    mission->SetPayment      ( PAYMENT_SPECIALMISSION_ARCINFILTRATION, PAYMENT_SPECIALMISSION_ARCINFILTRATION * 1.1 );
...
//    Computer *target = game->GetWorld ()->GetComputer ( "Darwin Research Associates Central Mainframe" );
	...
	for ( int i = 0; i < numfiles; ++i ) {
		...
		char datatitle [64];
		sprintf ( datatitle, "%c%c%c%c-DARWIN %d.dat", target->companyname [0], target->companyname [1],
	...
	sprintf ( description, SpecialMissionDescription (SPECIALMISSION_DARWIN) );
...
Mission *PlotGenerator::GenerateMission_SaveItForTheJury ()
{
    ...
    strcpy ( saveitforthejury_guytobeframed, poorsod->name );
    ...
    strcpy ( saveitforthejury_targetbankip, bank->ip );

	char description [SIZE_MISSION_DESCRIPTION];	
	ostrstream fulldetails;

    strcpy ( description, SpecialMissionDescription (SPECIALMISSION_SAVEITFORTHEJURY) );
...
Mission *PlotGenerator::GenerateMission_ShinyHammer ()
{
	...
	for ( int i = 0; i < 10; ++i ) {
		...
		char datatitle [64];
		sprintf ( datatitle, "%c%c%c%c-FAITH %d.dat", target->companyname [0], target->companyname [1],
												       target->companyname [2], target->companyname [3], i );
...
bool PlotGenerator::IsMissionComplete_ARCInfiltration ()
{
    ...
    if ( foundsize == 0 ) {
        ...
        completed_arcinfiltration = true;
        int change = 3 * ((float) foundsize / (float) 90.0);
...
bool PlotGenerator::IsMissionComplete_Darwin ()
{
    ...
    if ( foundsize == 0 ) {
        ...
        completed_darwin = true;
        int change = 2 * ((float) foundsize / (float) 90.0);
	...
	for ( int il = 0; il < comp->logbank.logs.Size (); ++il ) {
		if ( comp->logbank.logs.ValidIndex (il) ) {
			...
			if ( al->SUSPICIOUS != LOG_NOTSUSPICIOUS &&
				...
				UplinkAssert (vlf);
				Computer *fromcomp = vlf->GetComputer ();
				UplinkAssert (fromcomp);
...
bool PlotGenerator::IsMissionComplete_ShinyHammer ()
{
    ...
    if ( !computer->isrunning && computer->databank.formatted ) {
        ...
        msg->SetBody ( "Congratulations on deleting the data on Arunmor's central mainframe.  It appears you didn't "
        ...
        msg->SetBody ( "You don't appear to have damaged the Arunmor Central Mainframe very much.  Try again." );
        msg->Send ();
...
void PlotGenerator::Load  ( FILE *file )
{
	...
	fread ( act1scene3agent, sizeof(act1scene3agent), 1, file );
	fread ( act1scene4agent, sizeof(act1scene4agent), 1, file );
	fread ( act2scene1agent, sizeof(act2scene1agent), 1, file );

    fread ( &playervisitsplotsites, sizeof(playervisitsplotsites), 1, file );
	fread ( &playercancelsmail, sizeof(playercancelsmail), 1, file );
    fread ( &playerloyalty, sizeof(playerloyalty), 1, file );

    fread ( &completed_tracer, sizeof(completed_tracer), 1, file );
    fread ( &completed_takemetoyourleader, sizeof(completed_takemetoyourleader), 1, file );
    fread ( &completed_arcinfiltration, sizeof(completed_arcinfiltration), 1, file );
    fread ( &completed_darwin, sizeof(completed_darwin), 1, file );
    fread ( &completed_saveitforthejury, sizeof(completed_saveitforthejury), 1, file );
    fread ( &completed_shinyhammer, sizeof(completed_shinyhammer), 1, file );
	
    fread ( &specialmissionscompleted, sizeof(specialmissionscompleted), 1, file );

    fread ( saveitforthejury_guytobeframed, sizeof(saveitforthejury_guytobeframed), 1, file );
    fread ( saveitforthejury_targetbankip, sizeof(saveitforthejury_targetbankip), 1, file );
    fread ( tracer_lastknownip, sizeof(tracer_lastknownip), 1, file );

	fread ( &numuses_revelation, sizeof(numuses_revelation), 1, file );
    fread ( &revelation_releaseuncontrolled, sizeof(revelation_releaseuncontrolled), 1, file );
    fread ( &revelation_releasefailed, sizeof(revelation_releasefailed), 1, file );
    fread ( &revelation_arcbusted, sizeof(revelation_arcbusted), 1, file );

	fread ( &version_revelation, sizeof(version_revelation), 1, file );
	fread ( &version_faith, sizeof(version_faith), 1, file );

    LoadLList ( &infected, file );
...
void PlotGenerator::Save  ( FILE *file )
{
	...
	fwrite ( act1scene3agent, sizeof(act1scene3agent), 1, file );
	fwrite ( act1scene4agent, sizeof(act1scene4agent), 1, file );
	fwrite ( act2scene1agent, sizeof(act2scene1agent), 1, file );
    ...
    fwrite ( &saveitforthejury_guytobeframed, sizeof(saveitforthejury_guytobeframed), 1, file );
    fwrite ( &saveitforthejury_targetbankip, sizeof(saveitforthejury_targetbankip), 1, file );
    fwrite ( tracer_lastknownip, sizeof(tracer_lastknownip), 1, file );
