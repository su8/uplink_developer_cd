...
Mission *MissionGenerator::Generate_StealSingleFile ( Company *employer, Computer *target )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_STEALFILE, DIFFICULTY_MISSION_VARIANCE );
	...
	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_STEALFILE,
																difficulty * PAYMENT_MISSION_STEALFILE 
																		   * PAYMENT_MISSION_VARIANCE );
	int minrating		= difficulty;

	int acceptrating	= minrating + NumberGenerator::RandomNumber ( 2 );
	int datasize		= NumberGenerator::RandomNormalNumber ( difficulty, difficulty / 3.0 ) + 1;
	...
	int encrypted = difficulty < 3 ? 0 : NumberGenerator::RandomNormalNumber ( difficulty, difficulty / 2 );
	...
	switch ( NumberGenerator::RandomNumber ( 3 ) + 1 ) {
		...
		case 1 :		sprintf ( description,  "Steal important data from a rival company's file server" );		break;
		case 2 :		sprintf ( description,  "Access a remote system and copy important data files" );			break;
		case 3 :		sprintf ( description,  "Gain access to a rival system and steal research files" );			break;
...
Mission *MissionGenerator::Generate_StealAllFiles ( Company *employer, Computer *target )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_STEALALLFILES, DIFFICULTY_MISSION_VARIANCE );
	...
	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_STEALALLFILES,
																difficulty * PAYMENT_MISSION_STEALALLFILES 
																		   * PAYMENT_MISSION_VARIANCE );
	...
	char username [12];
	sprintf ( username, "temp%c%c%c%c", 'a' + NumberGenerator::RandomNumber ( 26 ),
	...
	int numfiles = NumberGenerator::RandomNormalNumber ( 10, 5 );
	int encrypted = NumberGenerator::RandomNormalNumber ( difficulty, difficulty );
    if ( encrypted > 7 ) encrypted = 7;
	int totalsize = 0;
	...
	for ( int i = 0; i < numfiles; ++i ) {
		...
		char datatitle [64];
		sprintf ( datatitle, "%c%c%c%c-%s %d.dat", target->companyname [0], target->companyname [1],
												   target->companyname [2], target->companyname [3],
													missiontypestring [missiontype-1], i );

		int size = NumberGenerator::RandomNormalNumber ( 10, 5 );
		totalsize += size;
	...
	sprintf ( whoisthetarget, "The data is owned by %s", target->companyname );
	
	strcpy ( completionA, target->ip );
	strcpy ( completionB, "ALL" );
	strcpy ( completionC, ourcomp->ip );
	sprintf ( completionD, "%d %d", numfiles, totalsize );

	if ( missiontype == 1 ) {

		sprintf ( description, "Steal valuable scientific research documents" );

		strcpy ( completionE, "SCIENTIFIC" );
		...
		sprintf ( description, "Copy large and secure corporate database" );
		...
		strcpy ( completionE, "CORPORATE" );

	}
	else if ( missiontype == 3 ) {

		strcpy ( completionE, "CUSTOMER" );

		sprintf ( description, "Break into High Security System and steal customer records" );
		...
		strcpy ( completionE, "SOFTWARE" );

		sprintf ( description, "Copy proprietary source code database" );

		fulldetails << "As you may be aware we are currently in direct competition with another rival corporation "
					   "to develop a software product whcih will no doubt dominate the market once released.  "
...
Mission *MissionGenerator::Generate_DestroySingleFile ( Company *employer, Computer *target )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_DESTROYFILE, DIFFICULTY_MISSION_VARIANCE );
	...
	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_DESTROYFILE,
																difficulty * PAYMENT_MISSION_DESTROYFILE 
																		   * PAYMENT_MISSION_VARIANCE );
	...
	switch ( NumberGenerator::RandomNumber ( 3 ) + 1 ) {
		...
		case 1 :		sprintf ( description,  "Find and destroy crucial data on a mainframe" );				break;
		case 2 :		sprintf ( description,	"Break into a rival computer system and sabotage files" );		break;
		case 3 :		sprintf ( description,	"Hack into a computer and delete key files" );					break;
...
Mission *MissionGenerator::Generate_DestroyAllFiles ( Company *employer, Computer *target )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_DESTROYALLFILES, DIFFICULTY_MISSION_VARIANCE );
	...
	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_DESTROYALLFILES,
																difficulty * PAYMENT_MISSION_DESTROYALLFILES 
																		   * PAYMENT_MISSION_VARIANCE );
	...
	if ( type == 1 ) {
		...
		strcpy ( description, "Destroy scientific research currently in progress" );
		...
		strcpy ( completionC, "SCIENTIFIC" );

	}
	else if ( type == 2 ) {

		strcpy ( description, "Destroy corporate data of a rival company" );
		...
		strcpy ( completionC, "CORPORATE" );

	}
	else if ( type == 3 ) {

		strcpy ( description, "Destroy customer records on a sales system" );
		...
		strcpy ( completionC, "CUSTOMER" );

	}
	else if ( type == 4 ) {

		strcpy ( description, "Destroy rival software in development" );
...
Mission *MissionGenerator::Generate_FindData_FinancialRecord ( Company *employer, Computer *target )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_FINDDATA_FINANCIAL, DIFFICULTY_MISSION_VARIANCE );
	...
	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_FINDDATA_FINANCIAL,
																difficulty * PAYMENT_MISSION_FINDDATA_FINANCIAL 
																		   * PAYMENT_MISSION_VARIANCE );
    ...
    sprintf ( completionA, "%s %d", target->ip, taccount->accountnumber );
    sprintf ( completionC, "Unused" );
	...
	if ( missiontype == 1 ) {
		...
		sprintf ( description, "Find financial details of one of our associates" );
...
//		if ( !balance ) return NULL;
		...
		sprintf ( completionB, "%s", "Balance" );				
		
	}
	else if ( missiontype == 2 ) {

		sprintf ( description, "Client interested in financial data on an enemy" );
...
//		if ( !loan ) return NULL;
		...
		char taccountno [32];
		char raccountno [32];
		sprintf ( taccountno, "%d", taccount->accountnumber );
		sprintf ( raccountno, "%d", raccount->accountnumber );
		...
		int amount = NumberGenerator::RandomNormalNumber ( 1000000, 700000 );
		...
		sprintf ( description, "Trace a recent balance transfer" );
        ...
        sprintf ( completionB, "%s", "TraceTransfer" );        
		sprintf ( completionC, "%s", raccount->name );
	...
	char whoisthetarget [128];
	sprintf ( whoisthetarget, "Nobody you would know." );
...
Mission *MissionGenerator::Generate_ChangeData_AcademicRecord ( Company *employer )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_CHANGEDATA_ACADEMIC, DIFFICULTY_MISSION_VARIANCE );
	...
	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_CHANGEDATA_ACADEMIC,
																difficulty * PAYMENT_MISSION_CHANGEDATA_ACADEMIC 
																		   * PAYMENT_MISSION_VARIANCE );
	...
	if ( missiontype == 1 ) {		// Add a degree =====================================================
		...
		sprintf ( description, "Generate a University Degree for a friend" );		
		
		fulldetails	<< "Provide this person with the following University Degree:\n"					
					<< "UNIVERSITY DEGREE :\n"
					<< "   SUBJECT : " << degreesubject << "\n"
					<< "   CLASS   : " << degreelevel << "\n";
		strcpy ( completionC, "University" );
		strcpy ( completionD, degreesubject );
		sprintf ( completionE, "%d", degreelevel );

        sprintf ( whysomuchmoney, "We really need this qualification" );
		...
		sprintf ( description, "Improve our associates' academic standing" );
		fulldetails << "This person requires a first class degree.\n";
		strcpy ( completionC, "University" );
		sprintf ( completionD, "%d", 1 );						
		sprintf ( completionE, "%d", 1 );						// Always needs a 1st

        sprintf ( whysomuchmoney, "This qualification will be very useful to us" );
		...
		sprintf ( description, "Help us prove a fellow employee is over-rated" );
		fulldetails << "We feel this person scored slightly too highly at his degree."
					<< "Give him a 3rd class replacement.\n";
		strcpy ( completionC, "University" );
		sprintf ( completionD, "%d", 3 );						
		sprintf ( completionE, "%d", 3 );						// Always needs a 3rd

        sprintf ( whysomuchmoney, "We really dont like the target" );
		...
		switch ( NumberGenerator::RandomNumber ( 5 ) ) {
			...
			case 4 :		sprintf ( qualification, "Registered Uplink Agent" );		break;
		...
		char query [64];
		sprintf ( query, "Other - %s", qualification );
		...
		sprintf ( description, "Qualifications required for wealthy professional" );
		fulldetails << "This individual urgently requires a \n"
					<< "'" << qualification << "' Qualification adding to his record.";
		strcpy ( completionC, "Other" );
		strcpy ( completionD, qualification );
		strcpy ( completionE, qualification );

        sprintf ( whysomuchmoney, "This qualification is needed to gain entry" );
	...
	strcpy ( completionA, target->ip );
	strcpy ( completionB, person->name );
	...
	char whoisthetarget [128];
	sprintf ( whoisthetarget, "He's just a friend of ours." );
...
Mission *MissionGenerator::Generate_ChangeData_SocialSecurity ( Company *employer )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_CHANGEDATA_SOCIAL, DIFFICULTY_MISSION_VARIANCE );
	...
	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_CHANGEDATA_SOCIAL,
																difficulty * PAYMENT_MISSION_CHANGEDATA_SOCIAL 
																		   * PAYMENT_MISSION_VARIANCE );
	...
	if ( missiontype == 1 ) {
		...
		sprintf ( description, "Falsify a Social Security document" );
		fulldetails << "The man below is living too easily.  Change his Personal Status to read 'Deceased'.\n";
		strcpy ( completionC, "Personal Status" );
		sprintf ( completionD, "%s", "Deceased" );						
		sprintf ( completionE, "%s", "Deceased" );
		...
		sprintf ( description, "Create part of a new identity" );
		...
		strcpy ( completionC, "Personal Status" );
		sprintf ( completionD, "%s", "Employed" );						
		sprintf ( completionE, "%s", "Employed" );
	...
	char whoisthetarget [128];
	sprintf ( whoisthetarget, "You wouldn't recognise the name." );
...
Mission *MissionGenerator::Generate_ChangeData_CriminalRecord ( Company *employer )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_CHANGEDATA_CRIMINAL, DIFFICULTY_MISSION_VARIANCE );
	...
	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_CHANGEDATA_CRIMINAL,
																difficulty * PAYMENT_MISSION_CHANGEDATA_CRIMINAL 
																		   * PAYMENT_MISSION_VARIANCE );
	...
	if ( missiontype == 1 ) {
		...
		sprintf ( description, "Help to discredit one of our rivals" );
		...
		fulldetails << "We need " << personname << " discrediting so that any "
					<< "future checks on his background will come up negative.\n"
					<< "Open his Criminal Record and give him a conviction for "
					<< "Robbery.";

		strcpy ( completionC, "Convictions" );
		strcpy ( completionD, "Robbery" );
		strcpy ( completionE, "Robbery" );

		strcpy ( whoisthetarget, "We wouldn't want to give his name out just yet." );

	}
	else if ( missiontype == 2 ) {

		sprintf ( description, "Help to stop a hacker from ever working again" );
		...
		strcpy ( completionC, "Convictions" );
		strcpy ( completionD, "High tech fraud" );
		strcpy ( completionE, "High tech fraud" );

		strcpy ( whoisthetarget, "A hacker who has attacked us before." );

	}
	else if ( missiontype == 3 ) {

		sprintf ( description, "Clear a criminal record" );
		...
		strcpy ( completionC, "Convictions" );
		strcpy ( completionD, "None" );
		strcpy ( completionE, "None" );

		strcpy ( whoisthetarget, "Just a friend of ours who needs help." );
															 ...
															 BankAccount *sacc, BankAccount *tacc, int amount, Person *hacker )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_TRACEUSER_BANKFRAUD, DIFFICULTY_MISSION_VARIANCE );
	if ( difficulty < MINDIFFICULTY_MISSION_TRACEUSER_BANKFRAUD ) difficulty = MINDIFFICULTY_MISSION_TRACEUSER_BANKFRAUD;

	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_TRACEUSER,
																difficulty * PAYMENT_MISSION_TRACEUSER
																		   * PAYMENT_MISSION_VARIANCE );
	...
	sprintf ( description, "Trace an unauthorised money transfer" );
	...
	char whoisthetarget [128];
	sprintf ( whoisthetarget, "That's for you to find out." );
...
Mission *MissionGenerator::Generate_PayFine ( Person *person, Company *company, int amount, Date *duedate, char *reason )
{
	...
	char personname [SIZE_PERSON_NAME];					// Person to send completion email to
	sprintf ( personname, "internal@%s.net", company->name );	
	...
	sprintf ( completionA, "%s", person->name );
	if ( strcmp ( person->name, "PLAYER" ) == 0 ) sprintf ( completionA, "%s", game->GetWorld ()->GetPlayer ()->handle );

	sprintf ( completionB, "%s %d", bank->ip, account->accountnumber );
	sprintf ( completionC, "%d", amount );
	...
	sprintf ( description, "Pay a %dc fine", amount );
...
Mission *MissionGenerator::Generate_FrameUser ( Company *employer, Person *person )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_FRAMEUSER, DIFFICULTY_MISSION_VARIANCE );
	if ( difficulty < MINDIFFICULTY_MISSION_FRAMEUSER ) difficulty = MINDIFFICULTY_MISSION_FRAMEUSER;

	//
	// Set up the basic variables of the mission
	//

	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_FRAMEUSER,
																difficulty * PAYMENT_MISSION_FRAMEUSER 
																		   * PAYMENT_MISSION_VARIANCE );
	...
	strcpy ( completionA, person->name );
	...
	if ( missiontype == 1 ) {									// Frame a user for hacking
		...
		sprintf ( description, "Frame a user for computer crimes" );
		...
		strcpy ( completionB, "HACKING" );

	}
	else if ( missiontype == 2 ) {

		sprintf ( description, "Frame a man for bank fraud" );
		...
		strcpy ( completionB, "FINANCIAL" );
		strcpy ( completionC, bank->ip );
		...
		sprintf ( description, "Frame a user for destruction of data." );
		...
		strcpy ( completionB, "DESTROYALL" );
		strcpy ( completionC, target->ip );
	...
	m->SetPayment      ( payment, payment * 1.1 );
...
Mission *MissionGenerator::Generate_TraceHacker	( Computer *hacked, Person *hacker )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_TRACEUSER, DIFFICULTY_MISSION_VARIANCE );
	...
	sprintf ( contact, "internal@%s.net", companyname );
	...
	strcpy ( description, "Trace a hacker who recently broke into our systems" );
...
Mission *MissionGenerator::Generate_ChangeAccount ( Company *employer, Computer *source, Computer *target )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_CHANGEACCOUNT, DIFFICULTY_MISSION_VARIANCE );
	...
	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_CHANGEACCOUNT,
																difficulty * PAYMENT_MISSION_CHANGEACCOUNT 
																		   * PAYMENT_MISSION_VARIANCE );
	...
	int amount_to_transfer = NumberGenerator::RandomNormalNumber ( sourceaccount->balance / 2, sourceaccount->balance * 0.4 );			
	amount_to_transfer = int ( amount_to_transfer / 500 ) * 500;
    
    if ( amount_to_transfer <= 0 ) return NULL;
    if ( amount_to_transfer > sourceaccount->balance ) amount_to_transfer = sourceaccount->balance * 0.9;
	...
	sprintf ( completionA, "%s %d", source->ip, sourceaccount->accountnumber );
	sprintf ( completionB, "%s %d", target->ip, targetaccount->accountnumber );
	sprintf ( completionC, "%d", amount_to_transfer );
	
	char description [SIZE_MISSION_DESCRIPTION];
	ostrstream details;
	ostrstream fulldetails;

	sprintf ( description, "Our esteemed colleague wishes to make a donation." );
	...
	fulldetails << "Break into the following well known financial institute:\n"							
				...
				<< "\n\n"
				<< "Remove any and all traces of this transfer.\n"
				<< "Send a notice of completion to\n"
				<< personname << "\n"
				<< "\n"
				<< "END"
	...
	char whoisthetarget [128];
	sprintf ( whoisthetarget, "%s", source->name );
...
Mission *MissionGenerator::Generate_RemoveComputer ( Company *employer, Computer *target )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_REMOVECOMPUTER, DIFFICULTY_MISSION_VARIANCE );
	...
	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_REMOVECOMPUTER,
																difficulty * PAYMENT_MISSION_REMOVECOMPUTER 
																		   * PAYMENT_MISSION_VARIANCE );
	...
	sprintf ( description, "Elite agent required for destruction of computer system" );
	...
	sprintf ( completionA, "%s", target->ip );
	...
	char whoisthetarget [128];
	sprintf ( whoisthetarget, "The system is owned by %s", target->companyname );
...
Mission *MissionGenerator::Generate_RemoveUser ( Company *employer )
{
	...
	int difficulty = NumberGenerator::RandomNormalNumber ( MINDIFFICULTY_MISSION_REMOVEUSER, DIFFICULTY_MISSION_VARIANCE );
	...
	int payment			= NumberGenerator::RandomNormalNumber ( difficulty * PAYMENT_MISSION_REMOVEUSER,
																difficulty * PAYMENT_MISSION_REMOVEUSER 
																		   * PAYMENT_MISSION_VARIANCE );
	...
	sprintf ( description, "Highly skilled Agent required for removal job" );
...
bool MissionGenerator::IsMissionComplete_StealAllFiles ( Mission *mission, Person *person, Message *message )
{
    ...
    if      ( strcmp ( datatype, "SCIENTIFIC" ) == 0 )  strcpy ( missiontypestring, "rsrch" );
    else if	( strcmp ( datatype, "RESEARCH" ) == 0 )	strcpy ( missiontypestring, "rsrch" );          // Not needed anymore, but older user files may have this
	else if ( strcmp ( datatype, "CORPORATE" ) == 0 )	strcpy ( missiontypestring, "corp" );
	else if ( strcmp ( datatype, "CUSTOMER" ) == 0 )	strcpy ( missiontypestring, "custmr" );
	else if ( strcmp ( datatype, "SOFTWARE" ) == 0 )	strcpy ( missiontypestring, "softw" );
	else	UplinkAbort ( "Unrecognised data type" );

	char stolendatatitle [64];
	sprintf ( stolendatatitle, "%c%c%c%c-%s", target->companyname [0], target->companyname [1],
											  target->companyname [2], target->companyname [3],
											  missiontypestring );
	...
	for ( int i = 0; i < ourcomp->databank.GetDataSize (); ++i ) {
		if ( ourcomp->databank.GetDataFile (i) ) {
			...
			if ( thisfile->encrypted ) foundencrypted = true;
	...
	if ( foundsize == 0 ) {
		...
		char newmessage [256];
		sprintf ( newmessage, "We've reviewed your recent mission to copy data files.\n"
...
bool MissionGenerator::IsMissionComplete_FindFinancial  ( Mission *mission, Person *person, Message *message )
{
	...
	sprintf ( source_acc_s, "%d", source_acc );
    ...
    if ( strcmp ( field, "Balance" ) == 0 ) {
        ...
        char sbalance [64];
        sprintf ( sbalance, "%d", actualbalance );
        ...
        if ( strstr ( LowerCaseString(message->GetBody ()), personresponsible ) != NULL ) {
...
bool MissionGenerator::IsMissionComplete_ChangeAccount ( Mission *mission, Person *person, Message *message )
{
	...
	sprintf ( source_acc_s, "%d", source_acc );
	sprintf ( target_acc_s, "%d", target_acc );
	...
	bool success = source_account->HasTransferOccured ( source_ip, target_ip, target_acc, amount );
...
bool MissionGenerator::IsMissionComplete_Special ( Mission *mission, Person *person, Message *message )
{
    ...
    if ( strcmp ( mission->description, PlotGenerator::SpecialMissionDescription ( SPECIALMISSION_MOLE ) ) == 0 ) {
        ...
        if ( foundsize == 0 && foundprogram == false ) {
            ...
            int foundpercent = 100 * ((float) foundsize / (float) 50);
            int payment = PAYMENT_SPECIALMISSION_MOLE * 0.75 * ((float) foundpercent / (float) 100);
            if ( foundprogram ) payment += PAYMENT_SPECIALMISSION_MOLE * 0.25;
...
void MissionGenerator::MissionCompleted ( Mission *mission, Person *person, Message *message )
{
	...
	body << "Congratulations on your success.\n"
		 << "Funds transfered : " << mission->payment << "c\n"
		 << "\n"
