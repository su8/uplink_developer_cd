...
local char *currentbuttonname = NULL;           // Button mouse is currently over
...
static float currentX = 0.1;
static float currentY = 0.1;
...
LList <char *> *wordwraptext ( const char *string, int linesize ) 
{
	...
	float averagecharsize = ((float) GciTextWidth ( "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ" )) / 52.0;	
...
void text_draw ( Button *button, bool highlighted, bool clicked ) 
{
	...
	if ( !highlighted || !EclIsButtonEditable (button->name) ) {
		...
		if ( time(NULL) >= (curserflash - 1) ) {
			...
			char *newcaption = new char [strlen(button->caption) + 2];
			sprintf ( newcaption, "%s_", button->caption );
...
void textbutton_keypress ( Button *button, char key )
{
	...
	if ( key == 8 ) {
		...
		char *newcaption = new char [strlen(button->caption) + 2];
		sprintf ( newcaption, "%s%c", button->caption, key );
		button->SetCaption ( newcaption );
...
void buttonborder_draw ( Button *button, bool highlighted, bool clicked )
{
	...
	glBegin ( GL_QUADS );
		...
		glColor4f ( 0.2, 0.2, 0.4, ALPHA );			glVertex2i ( button->x, button->y );
		glColor4f ( 0.3, 0.3, 0.5, ALPHA );			glVertex2i ( button->x + button->width, button->y );
		glColor4f ( 0.2, 0.2, 0.4, ALPHA );			glVertex2i ( button->x + button->width, button->y + button->height );
		glColor4f ( 0.3, 0.3, 0.5, ALPHA );			glVertex2i ( button->x, button->y + button->height );
...
void superhighlight_draw ( Button *button, bool highlighted, bool clicked )
{
	...
	if ( time(NULL) >= (superhighlight_flash-1) ) {
		...
		glBegin ( GL_QUADS );
			...
			// Top
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x, button->y - 5 );
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x + button->width, button->y - 5 );
			glColor4f ( 1.0, 1.0, 0.5, ALPHA );		glVertex2i ( button->x + button->width, button->y );
			glColor4f ( 1.0, 1.0, 0.5, ALPHA );		glVertex2i ( button->x, button->y );

			// Right
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x + button->width + 5, button->y );
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x + button->width + 5, button->y + button->height );
			glColor4f ( 1.0, 1.0, 0.5, ALPHA );		glVertex2i ( button->x + button->width, button->y + button->height );
			glColor4f ( 1.0, 1.0, 0.5, ALPHA );		glVertex2i ( button->x + button->width, button->y );
	
			// Bottom
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x, button->y + button->height + 5 );
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x + button->width, button->y + button->height + 5 );
			glColor4f ( 1.0, 1.0, 0.5, ALPHA );		glVertex2i ( button->x + button->width, button->y + button->height );
			glColor4f ( 1.0, 1.0, 0.5, ALPHA );		glVertex2i ( button->x, button->y + button->height );

			// Left
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x - 5, button->y );
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x - 5, button->y + button->height );
			glColor4f ( 1.0, 1.0, 0.5, ALPHA );		glVertex2i ( button->x, button->y + button->height );
			glColor4f ( 1.0, 1.0, 0.5, ALPHA );		glVertex2i ( button->x, button->y );
	
			// Top left			
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x - 5, button->y - 5 );
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x - 5, button->y );
			glColor4f ( 1.0, 1.0, 0.5, ALPHA );		glVertex2i ( button->x, button->y );
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x, button->y - 5 );

			// Top right			
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x + button->width + 5, button->y - 5 );			
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x + button->width + 5, button->y );
			glColor4f ( 1.0, 1.0, 0.5, ALPHA );		glVertex2i ( button->x + button->width, button->y );
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x + button->width, button->y - 5 );

			// Bottom right					
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x + button->width + 5, button->y + button->height + 5 );
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x + button->width, button->y + button->height + 5 );
			glColor4f ( 1.0, 1.0, 0.5, ALPHA );		glVertex2i ( button->x + button->width, button->y + button->height );
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x + button->width + 5, button->y + button->height );

			// Bottom left								
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x - 5, button->y + button->height + 5 );			
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x, button->y + button->height + 5 );
			glColor4f ( 1.0, 1.0, 0.5, ALPHA );		glVertex2i ( button->x, button->y + button->height );
			glColor4f ( 0.0, 0.0, 0.0, ALPHA );		glVertex2i ( button->x - 5, button->y + button->height );
...
void superhighlight_draw ( Button *button, bool highlighted, bool clicked )
{
	...
	glBegin ( GL_QUADS );
        ...
        float timediff = (float) (superhighlight_flash - EclGetAccurateTime ()) / 1000.0;
        float fraction;
        if ( timediff <= 1.0 ) 
            fraction = timediff;
        else if ( timediff <= 2.0 )
            fraction = 2.0 - timediff;
        else
            fraction = 0.0;

		// Top
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + border, button->y );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + button->width - border, button->y );
		glColor4f ( fraction, fraction, fraction/2.0, ALPHA );		glVertex2i ( button->x + button->width - border, button->y + border );
		glColor4f ( fraction, fraction, fraction/2.0, ALPHA );		glVertex2i ( button->x + border, button->y + border );

		// Right
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + button->width, button->y + border );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + button->width, button->y + button->height - border );
		glColor4f ( fraction, fraction, fraction/2.0, ALPHA );		glVertex2i ( button->x + button->width - border, button->y + button->height - border );
		glColor4f ( fraction, fraction, fraction/2.0, ALPHA );		glVertex2i ( button->x + button->width - border, button->y + border );			

		// Bottom
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + border, button->y + button->height );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + button->width - border, button->y + button->height );
		glColor4f ( fraction, fraction, fraction/2.0, ALPHA );		glVertex2i ( button->x + button->width - border, button->y + button->height - border );
		glColor4f ( fraction, fraction, fraction/2.0, ALPHA );		glVertex2i ( button->x + border, button->y + button->height - border );

		// Left
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x, button->y + border );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x, button->y + button->height - border );
		glColor4f ( fraction, fraction, fraction/2.0, ALPHA );		glVertex2i ( button->x + border, button->y + button->height - border );
		glColor4f ( fraction, fraction, fraction/2.0, ALPHA );		glVertex2i ( button->x + border, button->y + border );

		// Top left			
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + border/2, button->y + border/2 );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + border, button->y );
		glColor4f ( fraction, fraction, fraction/2.0, ALPHA );		glVertex2i ( button->x + border, button->y + border );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x, button->y + border );
		...
		glColor4f ( fraction, fraction, fraction/2.0, ALPHA );		glVertex2i ( button->x + button->width - border, button->y + border );			
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + button->width - border, button->y );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + button->width - border/2, button->y + border/2 );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + button->width, button->y + border );

		// Bottom right					
		glColor4f ( fraction, fraction, fraction/2.0, ALPHA );		glVertex2i ( button->x + button->width - border, button->y + button->height - border );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + button->width, button->y + button->height - border );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + button->width - border/2, button->y + button->height - border/2 );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + button->width - border, button->y + button->height );

		// Bottom left								
		glColor4f ( fraction, fraction, fraction/2.0, ALPHA );		glVertex2i ( button->x + border, button->y + button->height - border );			
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + border, button->y + button->height );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x + border/2, button->y + button->height - border/2 );
		glColor4f ( 0.0, 0.0, 0.0, ALPHA );		                    glVertex2i ( button->x, button->y + button->height - border );

	glEnd ();

	if ( EclGetAccurateTime () >= superhighlight_flash )
		superhighlight_flash = EclGetAccurateTime () + 2000;
...
void button_highlight ( Button *button )
{
	...
	if ( !currentbuttonname || strcmp ( button->name, currentbuttonname ) != 0 ) {

//		char filename [128];
//		sprintf ( filename, "%ssounds/mouseover.wav", app->path );
//		SgPlaySound ( RsArchiveFileOpen ( filename ) );

		if ( currentbuttonname ) delete [] currentbuttonname;
		currentbuttonname = new char [strlen(button->name)+1];
		strcpy ( currentbuttonname, button->name );
...
void create_msgbox ( char *title, char *message )
{
...
{
    return xPos * windowScaleX;
}

int GetScaledYPosition ( int yPos )
{
    return yPos * windowScaleY;
}
