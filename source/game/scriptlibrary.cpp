...
void ScriptLibrary::Script11 ()
{
    ...
    if ( strcmp ( name, "Fill this in" ) == 0 ) {
        create_msgbox ( "Error", "You must enter a username" );
        return;
    }
...
void ScriptLibrary::Script15 ()
{
	...
	strcpy ( name, EclGetButton ( "name 0 0" )->caption );
...
void ScriptLibrary::Script31 ()
{
	...
	if ( !EclGetButton ( "start_localhost" ) ) {
	    ...
	    PhoneDialler pd;
		pd.Create ( 400, 170 );
		pd.DialNumber ( IP_UPLINKPUBLICACCESSSERVER );    
		pd.Remove ();
...
void ScriptLibrary::Script33 ()
{
	...
	strcpy ( name, EclGetButton ( "nametext 0 0" )->caption );
	strcpy ( password, EclGetButton ( "passwordtext 0 0" )->caption );
	strcpy ( password2, EclGetButton ( "passwordtext2 0 0" )->caption );
...
void ScriptLibrary::Script40 ()
{
	...
	if ( !EclGetButton ( "downloadingUOS" ) ) {
		...
		int progress = 100.0 * (float) button->width / 200.0;

		char caption [8];
		sprintf ( caption, "%d %%", progress );
	...
	*/
	...
	if ( EclGetButton ( "syscheckUOS_13" ) ) {
		...
		for ( int i = 0; i < 14; ++i ) {
			...
			char buttonname [32];
			sprintf ( buttonname, "syscheckUOS_%d", i );
			...
			if ( !EclGetButton ( buttonname ) ) {
				...
				if ( i == 0 ) {
					...
					char okname [32];
					sprintf ( okname, "syscheckUOS_OK_%d", (i-1) );
				...
				switch ( i ) {
					case 0		:		sprintf ( caption, "Checking gateway HARDWARE : Trinity Ax086 2GQs cpu..." );			break;
					case 1		:		sprintf ( caption, "Checking gateway HARDWARE : K256 512KQds modem..." );				break;
					case 2		:		sprintf ( caption, "Checking gateway HARDWARE : 24 GQds DR-RAM memory..." );			break;
					case 3		:		sprintf ( caption, "Verifying integrity of Uplink Operating System..." );				break;
					case 4		:		sprintf ( caption, "Verifying security of Uplink Operating System..." );				break;
					case 5		:		sprintf ( caption, "Installing UPLINK Operating system files..." );						break;
					case 6		:		sprintf ( caption, "Initialising TSR programs in memory..." );							break;
					case 7		:		sprintf ( caption, "Sending RUN signal to core Uplink Kernel..." );						break;
					case 8		:		sprintf ( caption, "Checking Kernel Initilisation..." );								break;
					case 9		:		sprintf ( caption, "Establishing Network communications protocols..." );				break;
					case 10		:		sprintf ( caption, "Installing File-Copier..." );										break;
					case 11		:		sprintf ( caption, "Installing File-Deleter..." );										break;
					case 12		:		sprintf ( caption, "Installing Task-Manager you can read fast..." );					break;
					case 13		:		sprintf ( caption, "Running Graphical User Interface..." );								break;
				}
	...
	*/
	...
	if ( EclGetButton ( "syscheckUOS_0" ) ) {
		...
		for ( int i = 0; i < 14; ++i ) {
			...
			char okname [32];
			sprintf ( okname, "syscheckUOS_OK_%d", i );
	...
	*/
	...
	if ( EclGetButton ( "newpatch_3" ) ) {
		...
		for ( int i = 0; i < 4; ++i ) {
			...
			char buttonname [32];
			sprintf ( buttonname, "newpatch_%d", i );
			...
			if ( !EclGetButton ( buttonname ) ) {
                ...
                if ( i == 0 ) {
				    ...
				    char okname [32];
				    sprintf ( okname, "newpatch_OK_%d", (i-1) );
				...
				switch ( i ) {
					case 0		:		sprintf ( caption, "Reading new patch data..." );			break;
					case 1		:		sprintf ( caption, "Modifying Uplink executable..." );				break;
					case 2		:		sprintf ( caption, "Modifying Uplink runtime data..." );			break;
					case 3		:		sprintf ( caption, "Validating patch authorisation with Uplink Corporation..." );				break;
				}
...
void ScriptLibrary::Script46 ()
{
	...
	char okname [32];
	sprintf ( okname, "newpatch_OK_%d", 3 );
...
void ScriptLibrary::Script47 ()
{
    ...
    EclRegisterButton ( 150, 250, 300, 15, "Set the in-game screen resolution to 800x600", "This is now the recommended in-game resolution.", "newpatch_screenres" );
		...
		if ( system->TYPE == LANSYSTEM_AUTHENTICATION ) {
            ...
            if ( LanMonitor::IsInConnection( unlockIndex1 ) ||
                ...
                if ( comp->TYPE == COMPUTER_TYPE_LAN )
                {
                    ...
                    game->GetInterface()->GetRemoteInterface()->RunScreen ( system->screenIndex );
