...
void GameObituary::SetGameOverReason ( char *newreason )
{
	...
	strcpy ( gameoverreason, newreason );
...
void GameObituary::Evaluate ()
{
	...
	strcpy ( name, game->GetWorld ()->GetPlayer ()->handle );
    ...
    GatewayDef *gatewaydef = game->GetWorld ()->GetGatewayDef ( game->GetWorld ()->GetPlayer ()->gateway.type, 
...
void GameObituary::Load   ( FILE *file )
{
	...
	fread ( &uplinkrating, sizeof(uplinkrating), 1, file );
	fread ( &neuromancerrating, sizeof(neuromancerrating), 1, file );
	fread ( &specialmissionscompleted, sizeof(specialmissionscompleted), 1, file );

	fread ( &score_peoplefucked, sizeof(score_peoplefucked), 1, file );
	fread ( &score_systemsfucked, sizeof(score_systemsfucked), 1, file );
	fread ( &score_highsecurityhacks, sizeof(score_highsecurityhacks), 1, file );	
	fread ( &score, sizeof(score), 1, file );

    fread ( &demogameover, sizeof(demogameover), 1, file );
    fread ( &warezgameover, sizeof(warezgameover), 1, file );

	gameoverreason = LoadDynamicString ( file );
