	...
	if ( app->GetNetwork ()->STATUS == NETWORK_SERVER ) {
		...
		for ( i = 0; i < app->GetNetwork ()->GetServer ()->clients.Size (); ++i ) {
			if ( app->GetNetwork ()->GetServer ()->clients.ValidIndex (i) ) {
				...
				strcat ( cdetails, thisconnection );
