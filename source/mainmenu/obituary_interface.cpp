...
{	
	...
	if ( !IsVisible () ) {
		...
		char finances [32];
		sprintf ( finances, "%d credits", gob->money );

		char name [128];
		sprintf ( name, "Agent %s", gob->name );
        ...
        for ( int i = 0; i < 16; ++i ) {
            if ( gob->specialmissionscompleted & (1 << i) ) {
                ...
                char tooltip [128];
                char filename [128];
                sprintf ( name, "obituary_award %d", i );
                sprintf ( tooltip, "Completed Special Mission '%s'", PlotGenerator::SpecialMissionTitle (i) );
                sprintf ( filename, "awards/award%d.tif", i );
        ...
        sprintf ( numberoutof, "(%d out of %d)", numcompleted, 12 );
		...
		sprintf ( highsecurityhacks, "You compromised %d high security systems", gob->score_highsecurityhacks );
		sprintf ( livesruined, "You ruined the lives of %d people", gob->score_peoplefucked );
		sprintf ( systemsdestroyed, "You destroyed %d computers", gob->score_systemsfucked );
		...
		char score [64];
		sprintf ( score, "Final Score    %d", gob->score );
