...
void TheTeamInterface::NameDraw ( Button *button, bool highlighted, bool clicked )
{
	...
	for ( int i = 0; i < strlen(button->caption); ++i ) {
		char thischar [2];
		sprintf ( thischar, "%c", button->caption [i] );
		GciDrawText ( button->x + i * 12, ypos, thischar, HELVETICA_18 );
	}
...
void TheTeamInterface::TextDraw ( Button *button, bool highlighted, bool clicked )
{
	...
	for ( int line = 0; line < wrappedText->Size (); ++line ) {
		...
		for ( int i = 0; i < strlen(thisLine); ++i ) {
			char thischar [2];
			sprintf ( thischar, "%c", thisLine [i] );
			GciDrawText ( button->x + i * 6, ypos, thischar, HELVETICA_10 );
		}
...
void TheTeamInterface::Update ()
{
	...
	if ( targetindex != -1 ) {
		...
		if ( EclGetAccurateTime () > timesync ) {
			...
			timesync = EclGetAccurateTime () + 100;
...
void TheTeamInterface::MergeCaption ( char *buttonName, char *targetCaption )
{
	...
	if ( strlen(currentCaption)	< strlen(targetCaption) ) {
		...
		int newCaptionSize = strlen(currentCaption) + 2;
		newCaption = new char [newCaptionSize];
		strcpy ( newCaption, currentCaption );
		...
		int newCaptionSize = strlen(currentCaption);
		newCaption = new char [newCaptionSize];
	...
	int newCaptionSize = strlen(newCaption);
	int targetCaptionSize = strlen(targetCaption);
	int numchanges = 0;

	for ( int i = 0; i < newCaptionSize; ++i ) {
