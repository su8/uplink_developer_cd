...
void ClosingInterface::Update ()
{
    ...
    if ( EclGetAccurateTime () > timesync ) {
        ...
        switch ( status ) {
            ...
            case 1:
            {
                int captionNumber = (int) fmod(count, 7);
                button->SetCaption ( captions[captionNumber] );
                char filename [128];
                sprintf ( filename, "sounds/%d.wav", NumberGenerator::RandomNumber (9) + 1 );
                SgPlaySound ( RsArchiveFileOpen ( filename ), filename, true );
                timesync = EclGetAccurateTime () + 160;
                count++;
            ...
            case 3:
            {
                ...
                sprintf ( closingtime, "You've been using Uplink for %d minutes. Visit our web site to buy the full product.", playtime );
                ...
                timesync = EclGetAccurateTime () + 3000;
                status++;
                break;
