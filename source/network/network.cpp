...
#ifdef WIN32
	sprintf ( currentuser, "%s", getenv ( "USERNAME" ) );
	sprintf ( computer, "%s", getenv ( "COMPUTERNAME" ) );
#else
	sprintf ( currentuser, "%s", getenv ( "USER" ) );
	sprintf ( computer, "%s", getenv ( "HOST" ) );
#endif
...
char *Network::GetLocalIP ()
{
	...
	char *fullip = new char [64];
	sprintf ( fullip, "%u.%u.%u.%u", sip [0], sip [1], sip [2], sip [3] );
