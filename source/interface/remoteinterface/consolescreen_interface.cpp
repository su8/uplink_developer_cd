...
void ConsoleScreenInterface::PostClick ( Button *button )
{
	...
	if ( actualcommand ) {													// Copy actual command here
		...
		strcpy ( actualcommand, strchr (b->caption, '>')+1);				// Will be set to '' by the caption change
	}

	thisint->PutText ( 1, actualcommand );

	char newcaption [16];
	sprintf ( newcaption, "%s:>", thisint->currentdir );
...
void ConsoleScreenInterface::AddUser ( char *name )
{
	...
	if ( strcmp ( name, "System" ) == 0 ) {
		...
		char message [128];
		sprintf ( message, "User '%s' logged on", name );
...
void ConsoleScreenInterface::SetCurrentDir ( char *newcurrentdir )
{
	...
	char newcaption [16];
	sprintf ( newcaption, "%s:>", currentdir );
...
void ConsoleScreenInterface::PutText ( int userid, char *text )
{
	...
	if ( userid == 0 ) {
		...
		char msg [256];
		sprintf ( msg, "%s:>%s", currentdir, text );
...
void ConsoleScreenInterface::RunCommand ( ConsoleCommand *cc )
{
	...
	if ( cc->time > 0 ) {
		...
		timesync = EclGetAccurateTime () + cc->time;
		cc->time = 0;
...
void ConsoleScreenInterface::RunCommand_TEXT ( char *text )
{
	...
	for ( int i = 0; i < NUMLINES-1; ++i ) {
		...
		char name1 [32];
		char name2 [32];
		sprintf ( name1, "console_text %d", i );
		sprintf ( name2, "console_text %d", i + 1 );
	...
	char name1 [32];
	sprintf ( name1, "console_text %d", NUMLINES-1 );
		...
		if ( comp->databank.GetDataSize () == 0 ) {
			...
			for ( int i = 0; i < comp->databank.GetDataSize (); ++i ) {
				...
				if ( data ) {
					...
					sprintf ( filesummary, "%s    %dGq", data->title, data->size );
		...
		if ( strcmp ( currentdir, "sys" ) == 0 ) {
			...
			for ( int i = 0; i < comp->logbank.logs.Size (); ++i ) {
				if ( comp->logbank.logs.ValidIndex (i) ) {
                    ...
                    if ( al ) {
                        char caption [256];
                        sprintf ( caption, "Deleting log %s...", al->GetDescription() );
            ...
            for ( int i = 0; i < comp->databank.NumDataFiles(); ++i ) {
                ...
                if ( file ) {
                    char caption [256];
                    sprintf ( caption, "Deleting %s...", file->title );
...
void ConsoleScreenInterface::RunCommand_RUN	( char *program, bool actuallyrun )
{
	...
	if ( actuallyrun ) {
		...
		for ( int i = 0; i < comp->databank.GetDataSize (); ++i ) {
			...
			if ( data && strcmp ( LowerCaseString (data->title), program ) == 0 ) {
			...
			for ( int i = 0; i < comp->databank.GetDataSize (); ++i ) {
				...
				if ( data && strcmp ( LowerCaseString (data->title), program ) == 0 ) {
					...
					if ( data->TYPE == DATATYPE_PROGRAM ) {
						...
						char text [64];
						sprintf ( text, "Starting program %s...", data->title );
...
void ConsoleScreenInterface::Remove ()
{
	...
	if ( IsVisible () ) {
		...
		for ( int ti = 0; ti < NUMLINES; ++ti ) {
			...
			char name [32];
			sprintf ( name, "console_text %d", ti );
