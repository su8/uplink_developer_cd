...
void PasswordScreenInterface::Create ( ComputerScreen *newcs )
{
	...
	if ( !IsVisible () ) {
		...
		if ( btree ) {
			...
			EclRegisterButton ( 200, 315, 180, 130, "", "", "passwordscreen_codes" );
			EclRegisterButtonCallbacks ( "passwordscreen_codes", textbutton_draw, NULL, NULL, NULL );

			char message [256];
			sprintf ( message, "KNOWN ACCESS CODES\n\t%s", btree->data );
			EclRegisterCaptionChange ( "passwordscreen_codes", message, 1500 );
...
void PasswordScreenInterface::Update ()
{
	...
	if ( strcmp ( EclGetButton ( "passwordscreen_password" )->caption, GetComputerScreen ()->password ) == 0 ) {
