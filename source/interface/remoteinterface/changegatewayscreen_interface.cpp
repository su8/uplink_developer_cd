...
void ChangeGatewayScreenInterface::GatewayButtonDraw ( Button *button, bool highlighted, bool clicked )
{
	...
	GatewayDef *gd = game->GetWorld ()->GetGatewayDef (index, NULL);
	UplinkAssert (gd);
...
void ChangeGatewayScreenInterface::BuyConfirmClick ( Button *button )
{
	...
	cge->SetNewType (thisint->currentselect);
...
void ChangeGatewayScreenInterface::ScrollChange ( char *scrollname, int newIndex )
{
	...
	for ( int i = 0; i < numItems; ++i ) {
		...
		char bname [64];
		sprintf ( bname, "changegateway_title %d", i );
...
void ChangeGatewayScreenInterface::ShowGateway ( int index )
{
	...
	GatewayDef *gd = game->GetWorld ()->GetGatewayDef (currentselect, NULL);
    if ( gd ) {

	    char newstats1[128];
	    char newstats2[128];
	    sprintf ( newstats1, "Max CPUs : %d\nMax Memory : %dGq", gd->maxcpus, gd->maxmemory * 8 );
	    sprintf ( newstats2, "Max Security : %d\nBandwidth : %dGqs", gd->maxsecurity, gd->bandwidth );
	    ...
	    //char imagename [64];
	    //sprintf ( imagename, "gateway/gateway_t%d.tif", index );
...
int ChangeGatewayScreenInterface::GetGatewayPrice ( int index )
{
	...
	GatewayDef *oldgateway = game->GetWorld ()->GetGatewayDef ( game->GetWorld ()->GetPlayer ()->gateway.type, 
                                                                game->GetWorld ()->GetPlayer ()->gateway.name );
	UplinkAssert (oldgateway);

	GatewayDef *newgateway = game->GetWorld ()->GetGatewayDef (index, NULL);
	UplinkAssert (newgateway);
	
	return newgateway->cost - (oldgateway->cost * GATEWAY_PARTEXCHANGEVALUE );
...
void ChangeGatewayScreenInterface::Create ( ComputerScreen *newcs )
{
	...
	if ( !IsVisible () ) {
		...
		for ( int i = 0; i < numRows; ++i ) {
			...
			GatewayDef *gd = game->GetWorld ()->GetGatewayDef (i, NULL);
			UplinkAssert (gd);
...
//		EclGetButton ("changegateway_picture")->image_standard->Scale ( 200, 140 );
        ...
        GatewayDef *gd = game->GetWorld ()->GetGatewayDef( game->GetWorld ()->GetPlayer ()->gateway.type,
                                                           game->GetWorld ()->GetPlayer ()->gateway.name );
		ShowGateway ( gd->index );
...
void ChangeGatewayScreenInterface::CreateAreYouSure ()
{
	...
	GatewayDef *newgateway = game->GetWorld ()->GetGatewayDef (currentselect, NULL);
	UplinkAssert (newgateway);
	...
	if ( oldgateway->index > newgateway->index ) 
