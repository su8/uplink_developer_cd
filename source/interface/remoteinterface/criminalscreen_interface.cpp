...
void CriminalScreenInterface::ArrestClick ( Button *button )
{
	...
	if ( csi->recordindex != -1 ) {
		...
		bool brokenparole = strstr ( convictions, "parole" ) || strstr ( convictions, "Parole" );
...
void CriminalScreenInterface::UpdateScreen ()
{
	...
	if ( IsVisible () ) {
		...
		if ( person ) {
			...
			char filename [32];
			sprintf ( filename, "photos/image%d.tif", person->photoindex );			
...
void CriminalScreenInterface::Update ()
{
	...
	if ( searchname && IsVisible () ) {
		...
		int timems = EclGetAccurateTime () - lastupdate;
