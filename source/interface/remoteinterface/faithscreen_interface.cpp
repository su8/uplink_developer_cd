...
void FaithScreenInterface::Create ( ComputerScreen *newcs )
{
	...
	if ( !IsVisible () ) {
		...
		sprintf ( faith, "Version %1.1f", game->GetWorld ()->plotgenerator.GetVersion_Faith () );
		sprintf ( revelation, "Version %1.1f", game->GetWorld ()->plotgenerator.GetVersion_Revelation () );
