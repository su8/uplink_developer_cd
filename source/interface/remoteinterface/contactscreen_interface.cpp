...
void ContactScreenInterface::PutMessage ( int userid, char *message )
{
	...
	thisint->messagequeue.PutData ( fullmessage.str () );
...
void ContactScreenInterface::AddUser ( char *name )
{
	...
	if ( strcmp ( name, "System" ) == 0 ) {
		...
		char message [128];
		sprintf ( message, "User '%s' has entered the conference", name );
...
void ContactScreenInterface::AddQuestion ( char *question, int index )
{
	...
	char name1 [32];
	char name2 [32];
	sprintf ( name1, "contact_questionb %d", index );
	sprintf ( name2, "contact_question %d", index );
		...
		case CONTACT_TYPE_MISSION:
			...
			switch ( index ) {
				...
				case 2:									// How secure is the target
					...
					if ( thisint->mission->GetHowSecure () ) {
						...
						switch ( thisint->mission->difficulty ) {
							...
							case 10:	PutMessage ( 2, "Security will be extremely high." );			break;

							default:	UplinkAbort ( "Unrecognised mission difficulty" );
					...
					if ( thisint->askedpayment ) {
						...
						if ( chance >= 100 ||
							...
							if ( thisint->mission->maxpayment == 0 ||
								...
								char message [128];
								sprintf ( message, "OK, we'll give you %dc because of your reputation.", thisint->mission->maxpayment );
				...
				case 6:									// OK, agreed
				{
					...
					UplinkAssert ( cu );
					int index = cu->missions.FindData ( thisint->mission );
					UplinkAssert ( index != -1 );
					cu->missions.RemoveData ( index );

					// Run the bulletin board

					game->GetInterface ()->GetRemoteInterface ()->RunScreen ( thisint->GetComputerScreen ()->nextpage );
...
void ContactScreenInterface::Remove ()
{
	...
	if ( IsVisible () ) {
		...
		for ( int ti = 0; ti < NUMLINES; ++ti ) {
			...
			char name [32];
			sprintf ( name, "contact_text %d", ti );
...
*/
		...
		for ( int qi = 0; qi < numquestions; ++qi ) {
			...
			char name [32];
			sprintf ( name, "contact_question %d", qi );
			EclRemoveButton ( name );

			sprintf ( name, "contact_questionb %d", qi );
			EclRemoveButton ( name );
...
void ContactScreenInterface::Update ()
{
	...
	if ( !waiting && messagequeue.Size () > 0 ) {
		...
		for ( int i = 0; i < NUMLINES-1; ++i ) {
			...
			char name1 [32];
			char name2 [32];
			sprintf ( name1, "contact_text %d", i );
			sprintf ( name2, "contact_text %d", i + 1 );
		...
		int time = strlen(message) * 5;

		char name1 [32];
		sprintf ( name1, "contact_text %d", NUMLINES-1 );
