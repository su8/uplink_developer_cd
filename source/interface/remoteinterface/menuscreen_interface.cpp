...
void MenuScreenInterface::ClickMenuScreenOption ( Button *button )
{
	...
	char text [32];
	int nextpage, unused;
	sscanf ( button->name, "%s %d %d", text, &unused, &nextpage );	
...
void MenuScreenInterface::Create ( ComputerScreen *newcs )
{
	...
	if ( !IsVisible () ) {
		...
		int timeinterval = 1000.0 / GetComputerScreen ()->NumOptions ();
		...
		for ( int i = 0; i < GetComputerScreen ()->NumOptions (); ++i ) {
			...
			if ( game->GetInterface ()->GetRemoteInterface ()->security_level <= GetComputerScreen ()->GetSecurity (i) ) {
				...
				char bname1 [32], bname2 [32];
				sprintf ( bname1, "menuscreenimage %d %d", i, GetComputerScreen ()->GetNextPage ( i ) );
				sprintf ( bname2, "menuscreentext %d %d", i, GetComputerScreen ()->GetNextPage ( i ) );
