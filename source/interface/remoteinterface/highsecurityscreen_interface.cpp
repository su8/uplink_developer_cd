...
void HighSecurityScreenInterface::SystemTitleDraw ( Button *button, bool highlighted, bool clicked )
{
	...
	if ( mso->security == 10 )	glColor3f ( 0.2, 0.2, 0.7 );
	...
	if ( mso->security == 10 )	glColor3f ( 1.0, 1.0, 1.0 );
...
void HighSecurityScreenInterface::BypassClick ( Button *button )
{
	...
	if ( hs->nextpage != -1 )
		game->GetInterface ()->GetRemoteInterface ()->RunScreen ( hs->nextpage );
...
void HighSecurityScreenInterface::Create ( ComputerScreen *newcs )
{
	...
	if ( !IsVisible () ) {
		...
		for ( int i = 0; i < hs->systems.Size (); ++i ) {
			...
			sprintf ( title, "highsecurity_systemtitle %d", i );
			sprintf ( button, "highsecurity_systembutton %d", i );
			sprintf ( status, "highsecurity_systemstatus %d", i );
