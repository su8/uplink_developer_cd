...
void MessageScreenInterface::Click ( Button *button )
{
	...
	int nextpage;
	sscanf ( button->name, "messagescreen_click %d", &nextpage );
...
void MessageScreenInterface::Create ( ComputerScreen *newcs )
{
	...
	if ( !IsVisible () ) {
		...
		if ( GetComputerScreen ()->buttonmessage ) {
			...
			int width = 20 + strlen (GetComputerScreen ()->buttonmessage) * 10;
			char name [128];
			sprintf ( name, "messagescreen_click %d", GetComputerScreen ()->nextpage );
