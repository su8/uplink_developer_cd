...
void SWSalesScreenInterface::ClickSWButton ( Button *button )
{
	...
	char oldname [128];
	sprintf ( oldname, "SWsale %d", currentselect - baseoffset );
...
void SWSalesScreenInterface::DrawSWButton ( Button *button, bool highlighted, bool clicked )
{
	...
	if ( sale ) {
		...
		char cost [32];
		char size [32];
		sprintf ( name, "%s", sale->title );
		sprintf ( version, "v%d.0", sv->data );
		sprintf ( cost, "%dc", sv->cost );
		sprintf ( size, "%dGq", sv->size );
...
void SWSalesScreenInterface::NextVersionClick ( Button *button )
{
	...
	if ( sale ) {
		...
		if ( sale->GetVersion ( currentversion+1 ) ) {
			...
			char buttonname [32];
			sprintf ( buttonname, "SWsale %d", currentselect - baseoffset );
