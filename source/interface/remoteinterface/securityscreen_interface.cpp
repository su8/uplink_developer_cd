...
int SecurityScreenInterface::lastupdate = 0;
...
void SecurityScreenInterface::SystemTitleDraw ( Button *button, bool highlighted, bool clicked )
{
	...
	if ( ss->enabled ) glColor3f ( 0.2, 0.2, 0.7 );
	...
	if ( ss->enabled )	glColor3f ( 1.0, 1.0, 1.0 );
		...
		if ( comp->security.NumSystems () == 0 ) {
			...
			for ( int i = 0; i < comp->security.NumSystems (); ++i ) {
				...
				if ( ss ) {
					...
					sprintf ( title, "securityscreen_systemtitle %d", i );
					sprintf ( level, "securityscreen_systemlevel %d", i );
					sprintf ( status, "securityscreen_systemstatus %d", i );

					char leveltext [8];
					sprintf ( leveltext, "level %d", ss->level );
