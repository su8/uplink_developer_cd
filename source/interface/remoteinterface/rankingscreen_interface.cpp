...
void RankingScreenInterface::ExitClick ( Button *button )
{
	...
	game->GetInterface ()->GetRemoteInterface ()->RunScreen ( sss->nextpage );
...
void RankingScreenInterface::Create ( ComputerScreen *newcs )
{
	...
	if ( !IsVisible () ) {
		...
		for ( int i = 0; i < 10; ++i ) {
			...
			sprintf ( name, "ranking_name %d", i );
			sprintf ( status, "ranking_status %d", i );
			...
			if ( sorted.ValidIndex (i) ) {
				...
				char cname [128];
				sprintf ( cname, "%2d  :  Agent %s", i + 1, agent->handle );
				
				char cstatus [32];
				if      ( agent->STATUS == PERSON_STATUS_NONE )		sprintf ( cstatus, "Active" );
				else if ( agent->STATUS == PERSON_STATUS_INJAIL )   sprintf ( cstatus, "In Jail" );
				else if ( agent->STATUS == PERSON_STATUS_DEAD )		sprintf ( cstatus, "Deceased" );
				else												sprintf ( cstatus, "Unknown" );
		...
		if ( playerrank >= 10 ) {
			...
			char cname [128];
			sprintf ( cname, "%2d  :  Agent %s", playerrank + 1, game->GetWorld ()->GetPlayer ()->handle );
