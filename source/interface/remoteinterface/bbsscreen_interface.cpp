...
void BBSScreenInterface::ClickBBSButton ( Button *button )
{
	...
	char oldname [128];
	sprintf ( oldname, "BBmessage %d", currentselect - baseoffset );
...
void BBSScreenInterface::DrawBBSButton ( Button *button, bool highlighted, bool clicked )
{
	...
	if ( mission ) {
		...
		if ( !encrypted ) {
			...
			float ratingdif = game->GetWorld ()->GetPlayer ()->rating.uplinkrating - mission->minuplinkrating;
			if ( ratingdif > 5.0 ) ratingdif = 5.0;
			
			sprintf ( subject, mission->description );
			sprintf ( date, "%s", mission->createdate.GetShortString () );

#ifdef DEMOGAME
            glColor4f ( 1.0 - ratingdif * 0.2, 1.0 - ratingdif * 0.2, 1.0 - ratingdif * 0.2, 1.0 );
#else
			glColor4f ( 1.0 - ratingdif * 0.1, 1.0 - ratingdif * 0.1, 1.0 - ratingdif * 0.1, 1.0 );
#endif

		}
		else {

			sprintf ( subject, "Encrypted (Insufficient Uplink Rating)" );
...
void BBSScreenInterface::ExitClick ( Button *button )
{
	...
	game->GetInterface ()->GetRemoteInterface ()->RunScreen ( bbs->nextpage );
...
void BBSScreenInterface::ContactClick ( Button *button )
{
	...
	if ( mission && bbs->contactpage != -1 ) {
		...
		if ( game->GetWorld ()->GetPlayer ()->rating.uplinkrating >= mission->minuplinkrating ) {
			...
			game->GetInterface ()->GetRemoteInterface ()->RunScreen ( bbs->contactpage );
