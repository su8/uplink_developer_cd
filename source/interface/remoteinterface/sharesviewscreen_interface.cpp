...
bool SharesViewScreenInterface::EscapeKeyPressed ()
{
	...
	char closename [32];
	sprintf ( closename, "sharesviewscreen_close %d", GetComputerScreen ()->nextpage );
...
void SharesViewScreenInterface::DrawPriceGraph ( Button *button, bool highlighted, bool clicked )
{
	...
	if ( thisint->companyname [0] != ' ' ) {
		...
		for ( int it = 0; it < 12; ++it ) {
			...
			char date [32];
			sprintf ( date, "%s, %d", Date::GetMonthName ( month + 1 ), month > monthnow ? yearnow - 1 : yearnow );
...
void SharesViewScreenInterface::SetCompanyName ( char *newcompanyname )
{
	...
	for ( int it = 0; it < 12; ++it ) {
		...
		char bdate [32];
		char bname [32];
		sprintf ( bdate, "sharesviewscreen_date %d", it );
		sprintf ( bname, "sharesviewscreen_numbers %d", it );

		char date [32];
		sprintf ( date, "%s, %d", Date::GetMonthName ( month + 1 ), month > monthnow ? yearnow - 1 : yearnow );
		char value [16];
		sprintf ( value, "%d c", sharehistory [month] );
...
void SharesViewScreenInterface::Create ( ComputerScreen *newcs )
{
	...
	if ( !IsVisible () ) {
		...
		for ( int i = 0; i < 12; ++i ) {
			...
			char date [32];
			char name [32];
			sprintf ( date, "sharesviewscreen_date %d", i );
			sprintf ( name, "sharesviewscreen_numbers %d", i );
