...
void NewsScreenInterface::ClickNewsButton ( Button *button )
{
	...
	char oldname [128];
	sprintf ( oldname, "news_story %d", currentselect - baseoffset );
...
void NewsScreenInterface::DrawNewsButton ( Button *button, bool highlighted, bool clicked )
{
	...
	if ( news ) {
		...
		char date [64];
        char subject [256];        
		sprintf ( subject, news->headline );
		sprintf ( date, "%s", news->date.GetShortString () );
        ...
        char details [256];
        if ( firstendline ) {
            strncpy ( details, news->GetDetails (), firstendline - fulldetails );
