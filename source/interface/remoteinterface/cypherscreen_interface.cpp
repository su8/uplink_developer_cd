...
void CypherScreenInterface::DrawCypher ( Button *button, bool highlighted, bool clicked )
{
	...
	for ( int i = 0; i < CYPHER_WIDTH; ++i ) {
		for ( int j = 0; j < CYPHER_HEIGHT; ++j ) {
			...
			int xpos = button->x + button->width * ( (float) i / (float) CYPHER_WIDTH );
			int ypos = button->y + 12 + button->height * ( (float) j / (float) CYPHER_HEIGHT );

			char text [2];
			sprintf ( text, "%c", thisint->cypher[i][j] );

			if ( thisint->cypherlock[i][j] ) {
				
				float shade = 0.2 + (float) (thisint->cypher [i][j] - '0') / 10.0;
...
void CypherScreenInterface::BypassClick ( Button *button )
{
	...
	if ( cs->nextpage != -1 )
		game->GetInterface ()->GetRemoteInterface ()->RunScreen ( cs->nextpage );
