...
void NuclearWarScreenInterface::DrawMainMap ( Button *button, bool highlighted, bool clicked )
{
    ...
    for ( int i = 0; i < nwsi->nukes.Size (); ++i ) {
        ...
        int timediff = EclGetAccurateTime () - nuke->time;
        ...
        if ( timediff > 10000 ) {
            ...
            float d = (float) (timediff - 3000) / 7000.0;

            int dX = nuke->x + button->x;
            int dY = nuke->y + button->y;            
            
            int sX = nuke->sx + ((nuke->x + button->x) - nuke->sx) * d;
            int sY = nuke->sy + ((nuke->y + button->y) - nuke->sy) * d;
            ...
            if ( !nuke->sound ) {
                char explosion [128];
                sprintf ( explosion, "sounds/explosion%d.wav", NumberGenerator::RandomNumber(3)+1 );
            ...
            float d = (float) timediff / 3000.0;
            int dX = nuke->sx + ((nuke->x + button->x) - nuke->sx) * d;
            int dY = nuke->sy + ((nuke->y + button->y) - nuke->sy) * d;

            float col = (float) (timediff) / 3000.0;
...
void NuclearWarScreenInterface::ClickLocation ( Button *button )
{
    ...
    nuke->sy = 0;
    nuke->x = NumberGenerator::RandomNormalNumber ( SX(pgl->x), 20 );
    nuke->y = NumberGenerator::RandomNormalNumber ( SY(pgl->y), 20 );
    nuke->time = EclGetAccurateTime ();
    nuke->sound = false;
...
void NuclearWarScreenInterface::Create ( ComputerScreen *newcs )
{
	...
	if ( !IsVisible () ) {
		...
		for ( int i = 0; i < NUM_PHYSICALGATEWAYLOCATIONS; ++i ) {
			...
			char bname [64];
			sprintf ( bname, "nuclearwar_location %d", i );

			char tooltip [128];
			sprintf ( tooltip, "Simulate attack on this site", pgl->city );
