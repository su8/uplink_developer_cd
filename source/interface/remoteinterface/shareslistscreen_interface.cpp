...
void SharesListScreenInterface::ShareClick ( Button *button )
{
	...
	if ( thisint->filteredlist.ValidIndex (fileindex) ) {
		...
		game->GetInterface ()->GetRemoteInterface ()->RunScreen ( thisint->GetComputerScreen ()->viewpage );
...
void SharesListScreenInterface::ShareDraw ( Button *button, bool highlighted, bool clicked )
{
    ...
    */
	...
	if ( companyname ) {
		...
		sprintf ( currentprice, "%d c", company->GetSharePrice () );

		char changeinprice [16];
		if ( company->GetShareChange () != 0 )
			sprintf ( changeinprice, "(%d%%)", company->GetShareChange () );
		else
			sprintf ( changeinprice, "-" );
		...
		if ( company->GetShareChange () < 0 ) glColor4f ( 1.0, 0.0, 0.0, 1.0 );
...
void SharesListScreenInterface::ScrollUpClick ( Button *button )
{
	...
	for ( int i = 0; i < 14; ++i ) {
		...
		char name [128];
		sprintf ( name, "shareslistscreen_share %d", i );
		EclDirtyButton ( name );
...
void SharesListScreenInterface::ApplyFilter ( char *filter )
{
	...
	for ( int ib = 0; ib < 14; ++ib ) {
		...
		char name [128];
		sprintf ( name, "shareslistscreen_share %d", ib );
		EclDirtyButton ( name );
...
void SharesListScreenInterface::Create ( ComputerScreen *newcs )
{
	...
	if ( !IsVisible () ) {
		...
		for ( int li = 0; li < 14; ++li ) {
			...
			char name [128];
			sprintf ( name, "shareslistscreen_share %d", li );
