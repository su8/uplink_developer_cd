...
void LoansScreenInterface::IncreaseLoanClick ( Button *button )
{
	...
	if ( thisint->account && thisint->IsVisible () ) {
		
		// Make sure we don't take out too much

		int maxcredit = thisint->account->loan + game->GetWorld ()->GetPlayer ()->rating.creditrating * 100;
		...
		if ( thisint->account->loan < maxcredit ) {
			...
			char balance [16];
			sprintf ( balance, "%d", newbalance );
			EclGetButton ( "loansscreen_balance" )->SetCaption ( balance );
		
			// Update the loan

			char loan [16];
			sprintf ( loan, "%d", newloan );
			EclGetButton ( "loansscreen_loan" )->SetCaption ( loan );

			// Update the interest rate 

			char rate [16];
			if		( newloan <= SMALLLOAN_MAX )		sprintf ( rate, "(%d%% apr)", int (100.0 * SMALLLOAN_INTEREST) );
			else if ( newloan <= MEDIUMLOAN_MAX )		sprintf ( rate, "(%d%% apr)", int (100.0 * MEDIUMLOAN_INTEREST) );
			else if ( newloan <= LARGELOAN_MAX )		sprintf ( rate, "(%d%% apr)", int (100.0 * LARGELOAN_INTEREST) );
			else										sprintf ( rate, "(%d%% apr)", int (100.0 * MAXLOAN_INTEREST) );
...
void LoansScreenInterface::Create ( ComputerScreen *newcs )
{
	...
	if ( !IsVisible () ) {
		...
		if ( account ) {
			...
			char accno [16];
			sprintf ( accno, "%d", account->accountnumber );
			EclGetButton ( "loansscreen_accountnumber" )->SetCaption ( accno );

			// Update the balance

			char balance [16];
			sprintf ( balance, "%d", account->balance );
			EclGetButton ( "loansscreen_balance" )->SetCaption ( balance );
		
			// Update the loan

			char loan [16];
			sprintf ( loan, "%d", account->loan );
			EclGetButton ( "loansscreen_loan" )->SetCaption ( loan );

			// Update the interest rate 

			char rate [16];
			if		( account->loan <= SMALLLOAN_MAX )		sprintf ( rate, "(%d%% apr)", int (100.0 * SMALLLOAN_INTEREST) );
			else if ( account->loan <= MEDIUMLOAN_MAX )		sprintf ( rate, "(%d%% apr)", int (100.0 * MEDIUMLOAN_INTEREST) );
			else if ( account->loan <= LARGELOAN_MAX )		sprintf ( rate, "(%d%% apr)", int (100.0 * LARGELOAN_INTEREST) );
			else											sprintf ( rate, "(%d%% apr)", int (100.0 * MAXLOAN_INTEREST) );
		...
		char maxloan [32];
		sprintf ( maxloan, "Max loan: %dc", account->loan + game->GetWorld ()->GetPlayer ()->rating.creditrating * 100 );
