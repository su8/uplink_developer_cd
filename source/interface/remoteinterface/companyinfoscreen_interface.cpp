...
void CompanyInfoScreenInterface::Create ( ComputerScreen *newcs )
{
	...
	if ( !IsVisible () ) {
		...
		if ( boss && admin ) {
			...
			sprintf ( mdemail, "%s@%s.net", company->boss, companyname );
			sprintf ( adminemail, "%s@%s.net", company->administrator, companyname );
			sprintf ( mdtel, "tel. %s", boss->phonenumber );
			sprintf ( admintel, "tel. %s", admin->phonenumber );

		}
		else {

			sprintf ( mdemail, "Unlisted" );
			sprintf ( adminemail, "Unlisted" );
			sprintf ( mdtel, "Unlisted" );
			sprintf ( admintel, "Unlisted" );
