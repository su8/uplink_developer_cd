...
void SecurityBypass::MoveTo ( int x, int y, int time_ms )
{
	...
	if ( IsInterfaceVisible () ) {
		...
		sprintf ( main, "sbypass_main %d", pid );
		sprintf ( pause, "sbypass_pause %d", pid );
		sprintf ( close, "sbypass_close %d",  pid );
		sprintf ( title, "sbypass_title %d", pid );
		sprintf ( status, "sbypass_status %d", pid );
...
void SecurityBypass::CreateInterface ()
{
	...
	if ( !IsInterfaceVisible () ) {
		...
		EclRegisterButton ( 100, 100, 32, 32, "Bypass", "Click to use this proxy bypass", main );
		...
		EclRegisterButton ( 101, 101, 12, 12, "P", "Pause the proxy bypass", pause );
		...
		EclRegisterButton ( 119, 101, 12, 12, "", "Close the proxy bypass", close );
