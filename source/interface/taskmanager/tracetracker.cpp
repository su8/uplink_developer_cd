...
void TraceTracker::TraceDraw ( Button *button, bool highlighted, bool clicked )
{
    ...
    if ( game->GetWorld ()->GetPlayer ()->connection.TraceInProgress () ) {
        ...
        if ( thistask->beepdelay < 100 )
            ...
            int timepassedsincelastbeep = EclGetAccurateTime () - timeoflastbeep;
            brightness = 0.7 * ((float) timepassedsincelastbeep / (float) thistask->beepdelay);
        }
	...
	glBegin ( GL_QUADS );
		...
		glColor4f ( brightness, brightness, 0.7, 0.5 );
		glVertex2i ( button->x, button->y + button->height );

		glColor4f ( brightness, brightness, 0.4, 0.5 );
	...
	if ( highlighted || clicked )	glColor4f ( 1.0, 1.0, 1.0, 1.0 );
	else							glColor4f ( 1.0, 1.0, 1.0, ALPHA );    
...
void TraceTracker::AudioClick ( Button *button )
{
    ...
    char audioname [64];
    sprintf ( audioname, "tracetracker_audio %d", pid );
...
void TraceTracker::MoveTo ( int x, int y, int time_ms )
{
	...
	if ( IsInterfaceVisible () ) {
		...
		sprintf ( displayname, "tracetracker_display %d", pid );
		sprintf ( closename, "tracetracker_close %d", pid );
        sprintf ( audioname, "tracetracker_audio %d", pid );
...
void TraceTracker::Tick ( int n )
{
	...
	if ( IsInterfaceVisible () ) {
        ...
        if ( connection->TraceInProgress () && 
            ...
            traceestimate = EclGetAccurateTime () + timeremaining * 1000;
        ...
        if ( connection->TraceInProgress () ) {
            ...
            if ( EclGetAccurateTime () >= nextbeep ) {
                ...
                int timeremaining = traceestimate - EclGetAccurateTime ();
                beepdelay = (float) timeremaining / 20.0;
                nextbeep = EclGetAccurateTime () + beepdelay;
		...
		if ( connection->Traced () ) {
			...
			if ( version == 1.0 ) {
				...
				int percent = 100 * (float) connection->traceprogress / (float) (connection->GetSize () - 1);
				char caption [128];
				sprintf ( caption, "Trace: %d%%", percent );
				...
				char caption [128];
				sprintf ( caption, "Trace: %d secs", timeremaining );
				button->SetCaption ( caption );		
                ...
                int accuratetimeremaining = (traceestimate - EclGetAccurateTime ()) / 1000;
                if ( accuratetimeremaining < 0 ) accuratetimeremaining = 0;
                char caption [128];
                sprintf ( caption, "Trace: %ds (%d)", timeremaining, accuratetimeremaining );
                button->SetCaption ( caption );
