...
//	char sprogress [128];
//	sprintf ( sprogress, "proxydisable_progress %d", pid );
...
void ProxyDisable::Tick ( int n )
{
	...
	if ( IsInterfaceVisible () ) {
		...
		sprintf ( sprogress, "proxydisable_progress %d", SvbLookupPID ( this ) );
...
void ProxyDisable::CreateInterface ()
{
	...
	if ( !IsInterfaceVisible () ) {
		...
		sprintf ( stitle, "proxydisable_title %d", pid );
		sprintf ( sborder, "proxydisable_border %d", pid );
		sprintf ( sprogress, "proxydisable_progress %d", pid );
		sprintf ( sclose, "proxydisable_close %d", pid );	
