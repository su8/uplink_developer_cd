...
void FirewallDisable::GoClick ( Button *button )
{
	...
	strcpy ( thistask->ip, game->GetWorld ()->GetPlayer ()->remotehost );
...
//	char sprogress [128];
//	sprintf ( sprogress, "firewalldisable_progress %d", pid );
...
void FirewallDisable::Tick ( int n )
{
	...
	if ( IsInterfaceVisible () ) {
		...
		sprintf ( sprogress, "firewalldisable_progress %d", SvbLookupPID ( this ) );
		...
		if ( progress == 0 ) {												// Do nothing - waiting for user to click GO			
			...
			EclGetButton ( sprogress )->width = 40 * ((float) ticksdone / (float) ticksrequired);
...
//				EclRegisterCaptionChange ( sprogress, "Disabling..." );
			...
			EclGetButton ( sprogress )->width = 40 + 80 * ((float) ticksdone / (float) ticksrequired);
			EclDirtyButton ( sprogress );
...
void FirewallDisable::CreateInterface ()
{
	...
	if ( !IsInterfaceVisible () ) {
		...
		sprintf ( stitle, "firewalldisable_title %d", pid );
		sprintf ( sborder, "firewalldisable_border %d", pid );
		sprintf ( sprogress, "firewalldisable_progress %d", pid );
		sprintf ( sclose, "firewalldisable_close %d", pid );	
