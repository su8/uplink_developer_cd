...
void Defrag::CloseClick ( Button *button )
{
	...
	for ( int i = 0; i < thistask->transitData.Size(); ++i ) {
	
		DataBank *databank = &(game->GetWorld ()->GetPlayer ()->gateway.databank);
		UplinkAssert (databank);

		Data *data = thistask->transitData.GetData (i);
		UplinkAssert (data);
...
void Defrag::Tick ( int n )
{
	...
	if ( IsInterfaceVisible () ) {
		...
		sprintf ( sprogress, "defrag_progress %d", SvbLookupPID ( this ) );
		...
		if ( progress == 0 ) {												// Do nothing - waiting for user to click GO			
			...
			EclGetButton ( sprogress )->width = 120 * currentProgress;
			EclDirtyButton ( sprogress );

			int totalmemory = game->GetWorld ()->GetPlayer ()->gateway.databank.GetSize ();
			int newMemorySlot = totalmemory * currentProgress;
...
void Defrag::CreateInterface ()
{
	...
	if ( !IsInterfaceVisible () ) {
		...
		sprintf ( stitle, "defrag_title %d", pid );
		sprintf ( sborder, "defrag_border %d", pid );
		sprintf ( sprogress, "defrag_progress %d", pid );
		sprintf ( sclose, "defrag_close %d", pid );	
