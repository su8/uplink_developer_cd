...
void Decypher::MoveTo ( int x, int y, int time_ms )
{
	...
	sprintf ( stitle, "decypher_title %d", pid );
	sprintf ( sborder, "decypher_border %d", pid );
	sprintf ( sprogress, "decypher_progress %d", pid );
	sprintf ( sclose, "decypher_close %d", pid );	
...
void Decypher::SetTarget ( UplinkObject *uo, char *uos, int uoi )
{
	...
	if ( status == DECYPHER_OFF ) {
		...
		if ( uo->GetOBJECTID () == OID_CYPHERSCREEN ) {
			...
			int difficulty = CYPHER_WIDTH * CYPHER_HEIGHT * TICKSREQUIRED_BYPASSCYPHER * cs->difficulty;
	...
	if ( IsInterfaceVisible () ) {
		...
		if ( status == DECYPHER_OFF ) {
			...
			if ( progress >= numticksrequired ) {
				...
				int percentagedone = 100.0 * (float) progress / (float) numticksrequired;
...
void Decypher::CreateInterface ()
{
	...
	if ( !IsInterfaceVisible () ) {
		...
		EclRegisterButtonCallbacks ( sborder, BorderDraw, NULL, button_click, button_highlight );
		EclRegisterButtonCallback ( sborder, BorderClick );

		EclRegisterButton ( 285, 450, 120, 13, "Select target", "Shows the progress of the decypher", sprogress );
		EclRegisterButtonCallbacks ( sprogress, ProgressDraw, NULL, button_click, button_highlight );		
...
bool Decypher::IsInterfaceVisible ()
{
	...
	char stitle [128];
	sprintf ( stitle, "decypher_border %d", pid );
