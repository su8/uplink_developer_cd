...
void LogUnDeleter::MoveTo ( int x, int y, int time_ms )
{
	...
	sprintf ( stitle, "logundeleter_title %d", pid );
	sprintf ( sborder, "logundeleter_border %d", pid );
	sprintf ( sprogress, "logundeleter_progress %d", pid );
	sprintf ( sclose, "logundeleter_close %d", pid );	
...
bool LogUnDeleter::IsInterfaceVisible ()
{
	...
	char stitle [128];
	sprintf ( stitle, "logundeleter_border %d", pid );
