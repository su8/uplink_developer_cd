...
void LanScan::GoClick ( Button *button )
{
	...
	char sprogress [128];
	sprintf ( sprogress, "lanscan_progress %d", pid );
...
void LanScan::Tick ( int n )
{
	...
	if ( IsInterfaceVisible () ) {
		...
		char sprogress [128];
		sprintf ( sprogress, "lanscan_progress %d", SvbLookupPID ( this ) );
		...
		if ( status == LANSCAN_SCANNING ) {
			...
			int numSubnetsScanned = LAN_SUBNETRANGE * ((float) progress / (float) numticksrequired);
			...
			for ( int i = 0; i < lanComp->systems.Size (); ++i ) {
				if ( lanComp->systems.ValidIndex( i ) ) {
					...
					if ( system->subnet <= numSubnetsScanned &&
						...
						int difference = (version - 1) - system->security;
...
void LanScan::CreateInterface ()
{
	...
	if ( !IsInterfaceVisible () ) {
		...
		sprintf ( stitle, "lanscan_title %d", pid );
		sprintf ( sborder, "lanscan_border %d", pid );
		sprintf ( sprogress, "lanscan_progress %d", pid );
		sprintf ( sclose, "lanscan_close %d", pid );	
