...
void LanForce::MoveTo ( int x, int y, int time_ms )
{
	...
	sprintf ( stitle, "lanforce_title %d", pid );
	sprintf ( sborder, "lanforce_border %d", pid );
	sprintf ( sprogress, "lanforce_progress %d", pid );
	sprintf ( sclose, "lanforce_close %d", pid );	
...
void LanForce::CreateInterface ()
{
	...
	if ( !IsInterfaceVisible () ) {
		...
		sprintf ( sprogress, "lanforce_progress %d", pid );
		sprintf ( sclose, "lanforce_close %d", pid );	
		sprintf ( tooltip, "LAN Force v%1.1f", version );
...
bool LanForce::IsInterfaceVisible ()
{
	...
	char stitle [128];
	sprintf ( stitle, "lanforce_border %d", pid );
