...
void LogModifier::CommitClick ( Button *button )
{
	...
	sprintf ( nexttype, "logmodifier_nexttype %d",	pid );
	sprintf ( fromip,	"logmodifier_fromip %d",	pid );
	sprintf ( data1,	"logmodifier_data1 %d",		pid );
...
void LogModifier::ChangeLogType ( int pid )
{
	...
	char ltype	[64];
	sprintf ( ltype,	"logmodifier_ltype %d",		pid );
	EclGetButton ( ltype )->SetCaption ( logtypes [thistask->logtype+1] );

	char data1 [64];
	sprintf ( data1, "logmodifier_tdata1 %d", pid );
...
void LogModifier::MoveTo ( int x, int y, int time_ms )
{
	...
	if ( IsInterfaceVisible () ) {
		...
		sprintf ( stitle, "logmodifier_title %d", pid );
		sprintf ( sborder, "logmodifier_border %d", pid );
		sprintf ( sprogress, "logmodifier_progress %d", pid );
		sprintf ( sclose, "logmodifier_close %d", pid );	
		...
		if ( IsExpandedInterfaceVisible () ) {
			...
			sprintf ( background, "logmodifier_background %d",	pid );
			sprintf ( t_ltype,	  "logmodifier_tltype %d",		pid );
			sprintf ( ltype,	  "logmodifier_ltype %d",		pid );
			sprintf ( t_fromip,	  "logmodifier_tfromip %d",		pid );
			sprintf ( fromip,  	  "logmodifier_fromip %d",		pid );
			sprintf ( t_data1,	  "logmodifier_tdata1 %d",		pid );
			sprintf ( data1,	  "logmodifier_data1 %d",		pid );
			
			sprintf ( nexttype,	  "logmodifier_nexttype %d",	pid );
			sprintf ( commit,	  "logmodifier_commit %d",		pid );
			sprintf ( close,	  "logmodifier_close2 %d",		pid );
	...
	if ( status == LOGMODIFIER_STATUS_MODIFYING ) {
		...
		for ( int i = 0; i < n; ++i ) {
			...
			if ( progress >= numticksrequired ) {
				...
				log->SetTYPE   ( logtype );
				log->SetFromIP ( StripCarriageReturns ( b_fromip->caption ) );
				log->SetData1  ( StripCarriageReturns ( b_data1->caption ) );
...
void LogModifier::CreateInterface ()
{
	...
	if ( !IsInterfaceVisible () ) {
		...
		sprintf ( sprogress, "logmodifier_progress %d", pid );
		sprintf ( sclose, "logmodifier_close %d", pid );	
		sprintf ( tooltip, "Log Modifier v%1.1f", version );
...
void LogModifier::CreateExpandedInterface ()
{
	...
	if ( !IsExpandedInterfaceVisible () ) {
		...
		if ( source && source->logs.ValidIndex (sourceindex) ) {
			...
			char titlename [64];
			sprintf ( titlename, "logmodifier_title %d", pid );
...
bool LogModifier::IsExpandedInterfaceVisible ()
{
	...
	sprintf ( fromip_title, "logmodifier_background %d", pid );
