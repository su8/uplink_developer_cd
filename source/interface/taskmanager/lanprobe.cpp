...
void LanProbe::MoveTo ( int x, int y, int time_ms )
{
	...
	sprintf ( stitle, "lanprobe_title %d", pid );
	sprintf ( sborder, "lanprobe_border %d", pid );
	sprintf ( sprogress, "lanprobe_progress %d", pid );
	sprintf ( sclose, "lanprobe_close %d", pid );	
...
void LanProbe::Tick ( int n )
{
	...
	if ( IsInterfaceVisible () ) {
		...
		if ( status == LANPROBE_UNUSED ) {
			...
			EclGetButton ( sprogress )->width = 60 * ( (float) progress / (float) numticksrequired );
			...
			progress += n;
			UplinkAssert ( EclGetButton ( sprogress ) );
			EclGetButton ( sprogress )->width = 60 + 60 * ( (float) progress / (float) numticksrequired );
			EclDirtyButton ( sprogress );

            if ( LanMonitor::IsAccessable( systemIndex ) ) {

			    // Every link with a port number <= this has been scanned
			    int portDone = LAN_LINKPORTRANGE * ( (float) progress / (float) TICKSREQUIRED_SCANLANLINKS );
...
void LanProbe::CreateInterface ()
{
	...
	if ( !IsInterfaceVisible () ) {
		...
		sprintf ( sprogress, "lanprobe_progress %d", pid );
		sprintf ( sclose, "lanprobe_close %d", pid );	
		sprintf ( tooltip, "LAN Probe v%1.1f", version );
...
bool LanProbe::IsInterfaceVisible ()
{
	...
	char stitle [128];
	sprintf ( stitle, "lanprobe_border %d", pid );
