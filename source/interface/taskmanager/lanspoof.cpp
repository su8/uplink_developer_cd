...
void LanSpoof::MoveTo ( int x, int y, int time_ms )
{
	...
	sprintf ( stitle, "lanspoof_title %d", pid );
	sprintf ( sborder, "lanspoof_border %d", pid );
	sprintf ( sprogress, "lanspoof_progress %d", pid );
	sprintf ( sclose, "lanspoof_close %d", pid );	
...
void LanSpoof::CreateInterface ()
{
	...
	if ( !IsInterfaceVisible () ) {
		...
		sprintf ( sprogress, "lanspoof_progress %d", pid );
		sprintf ( sclose, "lanspoof_close %d", pid );	
		sprintf ( tooltip, "LAN Spoof v%1.1f", version );
...
bool LanSpoof::IsInterfaceVisible ()
{
	...
	char stitle [128];
	sprintf ( stitle, "lanspoof_border %d", pid );
