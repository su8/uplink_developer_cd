...
void Revelation::Tick ( int n )
{
    ...
    if ( EclGetAccurateTime() > animationtime + 50 ) {
        ...
        for ( int i = 0; i < numcopies; ++i ) {
            ...
            char bname [64];
            sprintf ( bname, "revelation %d %d %d", pid, nextnumber, i );
			...
			sprintf ( filename, "sounds/%d.wav", soundindex );
        ...
        animationtime = EclGetAccurateTime();
...
void Revelation::DrawRevelation ( Button *button, bool highlighted, bool clicked )
{
    ...
    if ( thistask ) {
        ...
        float shade = (float) difference / 40.0;
	...
	if ( IsInterfaceVisible () ) {
        ...
        for ( int i = 0; i < numcopies; ++i ) {
            for ( int j = 0; j < 40; ++j ) {
                ...
                char bname [64];
                sprintf ( bname, "revelation %d %d %d", pid, j, i );
                EclRemoveButton ( bname );
...
bool Revelation::IsInterfaceVisible ()
{
    ...
    int pid = SvbLookupPID (this);
    char bname [64];
    sprintf ( bname, "revelation %d %d %d", pid, 0, 0 );
