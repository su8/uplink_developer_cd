...
void IPProbe::GoClick ( Button *button )
{
	...
	sprintf ( name_display, "probe_display %d", pid );
	...
	if ( task->status == PROBE_IDLE ) {
		...
		task->timeout = time(NULL) + 1;						// (3 seconds to complete)
...
void IPProbe::CreateInterface ()
{
	...
	if ( !IsInterfaceVisible () ) {
		...
		sprintf ( name_go, "probe_go %d", pid );
		sprintf ( name_display, "probe_display %d", pid );
		sprintf ( name_close, "probe_close %d", pid );
