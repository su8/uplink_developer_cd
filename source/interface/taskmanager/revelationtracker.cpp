...
void RevelationTracker::Tick ( int n )
{
	...
	if ( time(NULL) > timesync + 3 ) {
        ...
        char boxname [128];
        sprintf ( boxname, "revelationtracker_box %d", pid );
...
void RevelationTracker::CreateInterface ()
{
	...
	if ( !IsInterfaceVisible () ) {
        ...
        sprintf ( titlename, "revelationtracker_title %d", pid );
        sprintf ( boxname, "revelationtracker_box %d", pid );
        sprintf ( closename, "revaltiontracker_close %d", pid );
