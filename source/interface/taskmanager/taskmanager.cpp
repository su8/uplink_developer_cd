...
void TaskManager::RunSoftware ( char *name, float version )
{
	...
	if		( strcmp ( name, "Password_Breaker" ) == 0 ) {		task = new PasswordBreaker (); }
		...
		printf ( "Task Manager warning : Called RunSoftware, name not recognised \n" );
		return;
	}
...
void TaskManager::Load ( FILE *file )
{
...
void TaskManager::Update ()
{
	...
	int timesincelastcycle = EclGetAccurateTime () - lastcycle;
