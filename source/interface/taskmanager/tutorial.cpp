...
void Tutorial::MoveTo ( int x, int y, int time_ms )
{
	...
	if ( IsInterfaceVisible () ) {
		...
		sprintf ( title, "tutorial_title %d", pid );
		sprintf ( skip, "tutorial_skip %d", pid );
		sprintf ( close, "tutorial_close %d", pid );
		sprintf ( text, "tutorial_text %d", pid );
		sprintf ( next, "tutorial_next %d", pid );
...
void Tutorial::MoveMenuTo ( int x, int y, int time_ms )
{
	...
	if ( IsMenuInterfaceVisible () ) {
		...
		sprintf ( part1, "tutorial_part1 1 %d", pid );
		sprintf ( part2, "tutorial_part2 2 %d", pid );
		sprintf ( part3, "tutorial_part3 3 %d", pid );
		sprintf ( part1button, "tutorial_part1button 1 %d", pid );
		sprintf ( part2button, "tutorial_part2button 2 %d", pid );
		sprintf ( part3button, "tutorial_part3button 3 %d", pid );
...
void Tutorial::RunTutorial1 ( int part )
{
	...
	if ( part == 1 ) {						// Tut 1 Part 1
		...
		// TODO : Check the player has an email
		...
		// TODO : Check the player has an email from Uplink Corporation
...
bool Tutorial::HasCompletedCurrentSection ()
{
	...
	if ( current_tutorial == 1 ) {
		...
		else if ( current_part == 22 )					return ( game->GetInterface ()->GetLocalInterface ()->InScreen () == SCREEN_EMAIL );
        ...
        if      ( current_part == 2 )                   return ( game->GetInterface ()->GetLocalInterface ()->InScreen () == SCREEN_MISSION );
                                                                 ...
                                                                 game->GetInterface ()->GetRemoteInterface ()->GetInterfaceScreen ()->ScreenID () == SCREEN_SWSALESSCREEN );
        else if ( current_part == 36 )                  return ( game->GetWorld ()->GetPlayer ()->gateway.databank.GetData ( "Trace_Tracker" ) != NULL && 
                                                                 game->GetWorld ()->GetPlayer ()->gateway.databank.GetData ( "Trace_Tracker" )->version >= 2.0 );
        ...
        else if ( current_part == 60 )                  return ( game->GetInterface ()->GetTaskManager ()->GetTargetProgramName () &&
