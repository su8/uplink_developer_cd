...
void AnalyserInterface::ConnectionDraw ( Button *button, bool highlighted, bool clicked )
{
	...
	int systemindex = 0;
	char name [32];
	sprintf ( name, "analyser_system 0" );
	...
	while ( EclGetButton ( name ) ) {
		...
		++systemindex;
		sprintf ( name, "analyser_system %d", systemindex );
...
void AnalyserInterface::Remove ()
{
	...
	if ( IsVisible () ) {
		...
		while ( EclGetButton ( name ) ) {
			...
			sprintf ( name, "analyser_system_t %d", systemindex );
			EclRemoveButton ( name );
...
void AnalyserInterface::Update ()
{
	...
	if ( strcmp ( remotehost, game->GetWorld ()->GetPlayer ()->remotehost ) != 0 ) {
		...
		strcpy ( remotehost, game->GetWorld ()->GetPlayer ()->remotehost );
		...
		if ( comp->security.NumSystems () == 1 ) {
			...
			button_assignbitmaps ( "analyser_system 0", "analyser/unknown.tif", "analyser/unknown.tif", "analyser/unknown.tif" );
			...
			button_assignbitmaps ( "analyser_system 1", "analyser/unknown.tif", "analyser/unknown.tif", "analyser/unknown.tif" );
			...
			button_assignbitmaps ( "analyser_system 2", "analyser/unknown.tif", "analyser/unknown.tif", "analyser/unknown.tif" );
	...
	if ( time(NULL) > lastupdate + 3 ) {
		...
		while ( EclGetButton ( name ) ) {
			...
			if ( ss && ss->enabled && !ss->bypassed ) 
				button_assignbitmaps ( name, "analyser/secure.tif", "analyser/secure_h.tif", "analyser/secure_c.tif" );

			else
				button_assignbitmaps ( name, "analyser/nonsecure.tif", "analyser/nonsecure_h.tif", "analyser/nonsecure_c.tif" );
