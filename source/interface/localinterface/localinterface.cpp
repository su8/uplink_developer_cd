...
void LocalInterface::Load ( FILE *file )
{

	LoadID ( file );

	fread ( &currentscreencode, sizeof(currentscreencode), 1, file );
...
void LocalInterface::Save ( FILE *file )
{
    ...
    // If we're looking at the LAN, don't save that fact
    // otherwise the window will be recreated on re-load, covering the screen

    if ( currentscreencode == SCREEN_LAN ) {                            
