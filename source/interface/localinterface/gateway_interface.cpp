...
void GatewayInterface::Create ()
{
	...
	if ( !IsVisible () ) {
		...
		GatewayDef *gatewaydef = game->GetWorld ()->GetGatewayDef ( gateway->type, gateway->name );
		UplinkAssert (gatewaydef);

		EclRegisterButton ( 20, 50, 600, 350, "", "", "gateway_background" );
		EclRegisterButtonCallbacks ( "gateway_background", DrawGatewayBackground, NULL, NULL, NULL );		

		EclRegisterButton ( 605, 52, 13, 13, "", "", "gateway_close" );
		...
		//char gatewayname [64];
		//sprintf ( gatewayname, "gateway/gateway%d.tif", gateway->type );
...
//		EclGetButton ("gateway_picture")->image_standard->Scale ( 256, 256 );
		...
		for ( int ic = 0; ic < gatewaydef->maxcpus; ++ic ) {
			...
			UplinkAssert (gdl);
			char bname [64];
			sprintf ( bname, "gateway_cpu %d", ic );
		...
		for ( int im = 0; im < gatewaydef->maxmemory; ++im ) {
			...
			char bname [64];
			sprintf ( bname, "gateway_memory %d", im );
		...
		for ( int is = 0; is < gatewaydef->maxsecurity; ++is ) {
			...
			char bname [64];
			sprintf ( bname, "gateway_security %d", is );
		...
		sprintf ( modemtooltip, "Modem (%dGqs)", gateway->GetBandwidth () );
...
void GatewayInterface::Remove ()
{
	...
	if ( IsVisible () ) {
		...
		for ( int ic = 0; ic < gatewaydef->maxcpus; ++ic ) {
			...
			sprintf ( bname, "gateway_cpu_caption %d", ic );
			EclRemoveButton ( bname );
		}
