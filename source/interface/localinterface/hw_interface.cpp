...
void HWInterface::CreateHWInterface ()
{
	...
	if ( !IsVisibleHWInterface () ) {
		...
		char cpustats [64];
		sprintf ( cpustats, "Total CPU speed: %d Ghz", gateway->GetCPUSpeed () );
		EclRegisterCaptionChange ( "hw_cpu",   cpustats );
	
		// Bandwidth

		char bandwidthstats [64];
		sprintf ( bandwidthstats, "Bandwidth: %d Gq/s", gateway->GetBandwidth () );
		EclRegisterCaptionChange ( "hw_modem", bandwidthstats );

		// Memory

		char memory [128];
		sprintf ( memory, "Memory Capacity: %d Gq", gateway->memorysize );
