    ...
    for ( int i = 0; i < lanComp->links.Size(); ++i ) {
        if ( lanComp->links.ValidIndex(i) ) {
            ...
            if ( link->visible > LANLINKVISIBLE_NONE ) {
                ...
                int fromX = background->x + (from->x) + fromSystem->width * link->fromX + offsetX;
                int fromY = background->y + (from->y) + fromSystem->height * link->fromY + offsetY;
                int toX = background->x + (to->x) + toSystem->width * link->toX + offsetX;
                int toY = background->y + (to->y) + toSystem->height * link->toY + offsetY;
				...
				if ( link->visible >= LANLINKVISIBLE_AWARE ) {
                    ...
                    glLineWidth ( 2.0 );
					DrawLink ( link, fromX, fromY, toX, toY );
                    glLineWidth ( 1.0 );
							...
							float toX, float toY )
{
    ...
    if ( (link->fromY == 1.0 && link->toY == 0.0) ||				  // Bottom to top
		...
		glBegin ( GL_LINE_STRIP );
			...
			if ( fromX > toX ) {
				glVertex2i ( fromX, fromY + (toY - fromY) * link->fromX );
				glVertex2i ( toX, fromY + (toY - fromY) * link->fromX );
			}
			else {
				glVertex2i ( fromX, fromY + (toY - fromY) * (1.0 - link->fromX) );
				glVertex2i ( toX, fromY + (toY - fromY) * (1.0 - link->fromX) );
			}
		...
		glBegin ( GL_LINE_STRIP );
			glVertex2i ( fromX, fromY );
            if ( fromY > toY ) {
                glVertex2i ( fromX + (toX - fromX) * link->fromY, fromY );
                glVertex2i ( fromX + (toX - fromX) * link->fromY, toY );
            }
            else {
                glVertex2i ( fromX + (toX - fromX) * (1.0 - link->fromY), fromY );
                glVertex2i ( fromX + (toX - fromX) * (1.0 - link->fromY), toY );
            }                
			glVertex2i ( toX, toY );
		glEnd ();
...
void LanInterface::SelectSystem ( int systemIndex )
{
    ...
    button_assignbitmap ( "lan_preview", intObj->filename );
...
void LanInterface::Create ()
{
    ...
    if ( !IsVisible () ) {
        ...
        strcpy ( ip, game->GetWorld ()->GetPlayer ()->remotehost );
...
void LanInterface::Update ()
{
	...
	if ( EclGetAccurateTime () > stippleupdate ) {
		...
		stippleupdate = EclGetAccurateTime () + 20;			
