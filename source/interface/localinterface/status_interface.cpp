...
//		EclRegisterButtonCallbacks ( "status_tneuromancerrating", text_draw, NULL, NULL, NULL );
        ...
        for ( int i = 0; i < 16; ++i ) {
            if ( game->GetWorld ()->plotgenerator.PlayerCompletedSpecialMission (i) ) {
                ...
                char tooltip [128];
                char filename [128];
                sprintf ( name, "status_award %d", i );
                sprintf ( tooltip, "Completed Special Mission '%s'", game->GetWorld ()->plotgenerator.SpecialMissionTitle (i) );
