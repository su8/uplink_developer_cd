	...
	if ( !IsVisibleFileList () ) {
		...
		if ( numfiles > 0 ) {
			...
			for ( int si = 0; si < numfiles; ++si ) {
				...
				sprintf ( caption, "%s", software.GetData (si) );
				sprintf ( tooltip, "Attach this file to the mail message" );
				sprintf ( name, "sendmail_file %d", si );
		...
		int index = 0;
		char bname [128];
		sprintf ( bname, "sendmail_file %d", index );
	...
	if ( !IsVisibleAddressBook () ) {
		...
		if ( numcontacts > 0 ) {
			...
			for ( int i = 0; i < numcontacts; ++i ) {
				...
				char name [64];
				sprintf ( name, "sendmail_addressbook %d", i );
		...
		int index = 0;
		char bname [128];
		sprintf ( bname, "sendmail_addressbook %d", index );
...
void SendMailInterface::Update ()
{
	...
	if ( changed && IsVisible () ) {
		...
		char attached [128];
		if ( m->GetData () ) sprintf ( attached, "File: %s", m->GetData ()->title );
		else		         sprintf ( attached, "File: None attached" );
