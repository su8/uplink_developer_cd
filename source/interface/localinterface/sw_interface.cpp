...
void SWInterface::SoftwareDraw ( Button *button, bool highlighted, bool clicked )
{
	...
	if ( nhighlighted || clicked )	glColor4f ( 1.0, 1.0, 1.0, 1.0 );
...
void SWInterface::ToggleSubMenu ( int softwareTYPE, int x, int y )
{
	...
	if ( !IsVisibleSubMenu ( softwareTYPE ) ) {
		...
		int timesplit = 500 / software.Size ();				// Ensures total time = 500ms

		for ( int si = 0; si < software.Size (); ++si ) {
	
			char caption [128], tooltip [128], name [128];
			sprintf ( caption, "%s v%1.1f", software.GetData (si), versions.GetData (si) );
			sprintf ( tooltip, "Runs this software application" );
			sprintf ( name, "hud_software %d", si );
		...
		char bname [32];
		sprintf ( bname, "hud_swmenu %d", currentsubmenu );
...
void SWInterface::SoftwareHighlight ( Button *button )
{
	...
	if ( currentprogrambutton != index ) {
        ...
        if ( IsVisibleVersionMenu ( NULL ) ) {
			...
			char bname [32];
			sprintf ( bname, "hud_software %d", currentprogrambutton );
			EclDirtyButton ( bname );
		}

		currentprogrambutton = index;
		timesync = EclGetAccurateTime ();
...
void SWInterface::Update ()
{
	...
	if ( currentprogrambutton != -1 &&
		...
		char correctname [32];
		sprintf ( correctname, "hud_software %d", currentprogrambutton );
