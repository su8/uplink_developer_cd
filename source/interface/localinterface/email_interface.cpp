...
void EmailInterface::EmailReply ( Button *button )
{
	...
	strcpy ( contact, em->message->from );
...
{	
	...
	if ( !IsVisible () ) {
		...
		char *subject = new char [strlen(message->GetSubject()) + 12];
		ostrstream body;

		sprintf ( from, "From : %s", message->from );
		sprintf ( date, "Date : %s", message->date.GetShortString () );
		sprintf ( subject, "Subject : %s", message->GetSubject () );
