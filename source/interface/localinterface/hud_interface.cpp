...
void HUDInterface::EmailHighlight ( Button *button )
{
	...
	if ( message ) {
		...
		char tooltip [512];
		sprintf ( tooltip, "'%s', From '%s'", message->GetSubject (), message->from );
...
void HUDInterface::MissionHighlight ( Button *button )
{
	...
	if ( mission ) {
		...
		char tooltip [512];
		sprintf ( tooltip, "%s", mission->description );
...
void HUDInterface::MoveSelecter ( int screenID, int screenindex )
{
	...
	switch ( screenID ) {
		...
		{
			char bname [32];
			sprintf ( bname, "hud_message %d", screenindex );
		...
		{
			char bname [32];
			sprintf ( bname, "hud_mission %d", screenindex );
			HighlightToolbarButton ( bname );
			break;
		}
...
void HUDInterface::HighlightToolbarButton   ( char *bname )
{
	...
	if ( button ) {
		...
		strcpy ( GetHUD ()->previoushighlight, bname );
...
void HUDInterface::Update ()
{
	...
	if ( IsVisible () ) {
        ...
        sprintf ( caption, "%s", game->GetWorld ()->GetPlayer ()->GetRemoteHost ()->ip );
		...
		for ( int mi = 0; mi < game->GetWorld ()->GetPlayer ()->messages.Size (); ++mi ) {
			...
			char bname [128];
			sprintf ( bname, "hud_message %d", mi );
		...
		char bname [128];
		sprintf ( bname, "hud_message %d", removeMessageIndex );
		...
		for ( int msi = 0; msi < game->GetWorld ()->GetPlayer ()->missions.Size (); ++msi ) {
			...
			char bname [128];
			sprintf ( bname, "hud_mission %d", msi );
        ...
        sprintf ( bname, "hud_mission %d", removeMissionIndex );
        while ( EclGetButton ( bname ) ) {
			EclRemoveButton ( bname );
            ++removeMissionIndex;
            sprintf ( bname, "hud_mission %d", removeMissionIndex );
