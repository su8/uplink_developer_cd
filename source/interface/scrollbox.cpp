...
void ScrollBox::DrawScrollBar ( Button *button, bool highlighted, bool clicked )
{
    ...
    if ( thisBox ) {
        ...
        int x = thisBox->x;
        int w = thisBox->w;
        int y = thisBox->y + 15 + ((float) thisBox->currentIndex / (float) thisBox->numItems) * (thisBox->h - 30);
        int h = ((float) thisBox->windowSize / (float) thisBox->numItems) * (thisBox->h - 30);
...
void ScrollBox::UpdateGrabScroll ()
{
    ...
    if ( thisBox ) {
        ...
        int barH = ((float) thisBox->windowSize / (float) thisBox->numItems) * (thisBox->h - 30);
        ...
        if (thisBox->numItems * fractionThrough <= (thisBox->numItems - thisBox->windowSize) ) {
            thisBox->SetCurrentIndex( thisBox->numItems * fractionThrough );        
        }
        else {
...
void ScrollBox::CreateInterface ()
{
    ...
    sprintf ( scrollUpName, "scrollup %s", name );
    sprintf ( scrollDownName, "scrolldown %s", name );
    sprintf ( scrollBarName, "scrollbar %s", name );
