...
/*==============================================================================
  ...
  $Id: drv_sam9407.c,v 1.5 2000/02/14 22:10:21 miod Exp $
...
static SWORD sampleLoad(SAMPLOAD *s, int type)
{
	...
	if(!(bankP->flags & SF_LOOP)) {
		memset(samples+bankP->length, 0, NO_LOOP_SAMPLES<<1);
	...
	if(ioctl(modfd, SAM_IOC_MOD_SAMPLES_LOAD, &modSamples)<0) {
		free(samples);
		bankP->inUse=0;
		_mm_errno=MMERR_SAMPLE_TOO_BIG;
		return -1;
...
static BOOL init(void)
{
	...
	if(ioctl(modfd, SAM_IOC_API_INFO, &apiInfo)<0 ||
	   apiInfo.version!=SAM_API_VERSION) {
		_mm_errno=MMERR_OPENING_AUDIO;
		return 1;
	}
...
{
	static int wait_echo, ticker_idx;
	...
	if(wait_echo)
		while(read(modfd, &event, sizeof(event))>0 && (int)event.echo.closure!=ticker_idx);
	wait_echo=1;
	ticker_idx++;
