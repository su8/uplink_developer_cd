...
#if defined(__NT__)
                        if (!mf->songname)
                            SetConsoleTitle("Mikdx playing"); else
                            SetConsoleTitle(mf->songname);
#endif
		/*
		...
		while(!MP_Ready()){
			...
			Sleep(10);
                        ...
                        /* no wait for djgpp ? */
                                    ...
                                    hlt();
#endif
                                    printf("\rsngpos:%d patpos:%d sngspd %d bpm %d  freeHwBuf %i ",
                                        mp_sngpos,mp_patpos,mp_sngspd,mp_bpm,freeHwBuffer());
		}
