...
void Svb_button_draw ( Button *button, bool highlighted, bool clicked )
{
	...
	glBegin ( GL_QUADS );
		...
		if ( clicked ) glColor4f ( 0.5, 0.5, 1.0, 0.6 );
		else if ( highlighted ) glColor4f ( 0.2, 0.2, 0.9, 0.6 );
		else glColor4f ( 0.0, 0.0, 0.4, 0.6 );
		glVertex3i ( button->x, button->y, 0 );

		if ( clicked ) glColor4f ( 0.7, 0.7, 1.0, 0.6 );
		else if ( highlighted ) glColor4f ( 0.5, 0.5, 1.0, 0.6 );
		else glColor4f ( 0.0, 0.0, 0.7, 0.6 );
...
void Svb_column_draw ( Button *button, bool highlighted, bool clicked )
{
	...
	float scale = (float) button->width / 50.0;
	if ( highlighted ) scale *= 2;

	glBegin ( GL_QUADS );

		glColor4f ( 0.0, 2.0 - scale, scale, 0.6 );
		glVertex3i ( button->x, button->y, 0 );
