							...
							void (*callback) () )
{
	...
	if ( button ) {
		...
		anim->time = time_ms;
		anim->starttime = EclGetAccurateTime ();              
		anim->finishtime = EclGetAccurateTime () + time_ms;
				...
				if ( EclGetAccurateTime () >= anim->finishtime || !animsenabled ) {
					...
					if ( anim->MOVETYPE == MOVE_STRAIGHTLINE ) {
						...
						if ( anim->dX || anim->dY ) {
							...
							int dX = (anim->sourceX + ( EclGetAccurateTime () - anim->starttime ) * anim->dX) - anim->button->x;
							int dY = (anim->sourceY + ( EclGetAccurateTime () - anim->starttime ) * anim->dY) - anim->button->y;
							...
							anim->button->x = anim->sourceX + ( EclGetAccurateTime () - anim->starttime ) * anim->dX;
							anim->button->y = anim->sourceY + ( EclGetAccurateTime () - anim->starttime ) * anim->dY;							
						...
						if ( anim->dX || anim->dY ) {
							...
							if ( EclGetAccurateTime () < halftime ) {
								float firsthalf = (float) ( EclGetAccurateTime () - anim->starttime ) / (float) ( halftime - anim->starttime );
								anim->button->y = anim->sourceY + (anim->targetY - anim->sourceY) * firsthalf;
							}
							else {
								float secondhalf = (float) ( EclGetAccurateTime () - halftime ) / (float) ( anim->finishtime - halftime );
								anim->button->y = anim->targetY;
								anim->button->x = anim->sourceX + (anim->targetX - anim->sourceX) * secondhalf;
							}
					...
					if ( anim->dW || anim->dH ) {
						...
						anim->button->width = anim->sourceW + ( EclGetAccurateTime () - anim->starttime ) * anim->dW;
						anim->button->height = anim->sourceH + ( EclGetAccurateTime () - anim->starttime ) * anim->dH;
